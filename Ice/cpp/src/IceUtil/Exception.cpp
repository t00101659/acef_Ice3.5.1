...
#if defined(__GNUC__) && !defined(__sun) && !defined(__FreeBSD__) && !defined(__MINGW32__) && !defined(__ANDROID__)
#  include <execinfo.h>
#  include <cxxabi.h>
...
IceUtil::SyscallException::ice_print(ostream& os) const
{
    ...
    if(_error != 0)
    {
		os << ":\nsyscall exception: " << "error:[" << _error << "]: " << IceUtilInternal::errorToString(_error);
    }
}
