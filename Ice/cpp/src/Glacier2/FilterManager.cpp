...
Glacier2::FilterManager::create(const InstancePtr& instance, const string& userId, const bool allowAddUser)
{
    ...
    LOG(CSystemLog::LOG_DEBUG, "Glacier2.Filter.Identity.Accept[%d]:%s.", 
        (int)allowIdSeq.size(), allowIdSeq.size() >0?instance->communicator()->identityToString(allowIdSeq[0]).c_str():"");
