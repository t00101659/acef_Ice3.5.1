...
NodeService::startImpl(int argc, char* argv[], int& status)
{
    ...
    LOG(CSystemLog::LOG_DEBUG, "[NodeService::startImpl] Create Activator success.");
    //
    ...
    else if(properties->getProperty("Ice.Default.Locator").empty())
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[NodeService::startImpl] property `Ice.Default.Locator' is not set");
        return false;
    }
    ...
    if(dataPath.empty())
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[NodeService::startImpl] property `IceGrid.Node.Data' is not set");
        return false;
    }
    else
    {
        if(!IceUtilInternal::directoryExists(dataPath))
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "[NodeService::startImpl] property `IceGrid.Node.Data' is set to an invalid path:%s",
                ex.what());
            return false;
        }
    ...
    if(properties->getProperty("IceGrid.Node.Endpoints").empty())
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[NodeService::startImpl] property `IceGrid.Node.Endpoints' is not set");
        return false;
    }
    ...
    if(name.empty())
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[NodeService::startImpl] property `IceGrid.Node.Name' is not set");
        return false;
    }
    ...
    //
    LOG(CSystemLog::LOG_DEBUG, "[NodeService::startImpl] Create the node object adapter");
    ...
    //
    LOG(CSystemLog::LOG_DEBUG, "[NodeService::startImpl] Create a new timer to handle server activation/deactivation timeouts");
    ...
    LOG(CSystemLog::LOG_DEBUG, "[NodeService::startImpl] create Node proxy :%s-%s", id.name.c_str(), id.category.c_str());
    ...
    LOG(CSystemLog::LOG_DEBUG, "[NodeService::startImpl] publish proxy:%s-%s.", nodeProxy->ice_getIdentity().name.c_str(),
        nodeProxy->ice_getIdentity().category.c_str());
    ...
    if(!nowarn)
    {
        ...
        catch(const Ice::LocalException& ex)
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "[NodeService::startImpl] couldn't reach the IceGrid registry (this is expected " 
                "if it's down, otherwise please check the value of the " 
                "Ice.Default.Locator property):%s", ex.what());
        }
    }
    ...
    //
    LOG(CSystemLog::LOG_DEBUG, "[NodeService::startImpl] Create the node sessions with the registries.");
    ...
    // Start the activator.
    //
    LOG(CSystemLog::LOG_DEBUG, "[NodeService::startImpl] Start the activator");
    _activator->start();

    //
    // Activate the adapter.
    //
    LOG(CSystemLog::LOG_DEBUG, "[NodeService::startImpl] Activate the adapter");
    _adapter->activate();
	...
	)
{
	...
	SYSTEMTIME	tm;
	GetLocalTime(&tm);
	std::string strPath = "./IceGrid_dump" + inttostring(tm.wHour) + inttostring(tm.wMinute) + inttostring(tm.wSecond) + ".dmp";
