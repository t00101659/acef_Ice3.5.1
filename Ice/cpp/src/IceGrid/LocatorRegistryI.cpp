                                              ...
                                              const Ice::Current&)
{
    ...
    catch(const Ice::Exception& ex)
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[LocatorRegistryI::setServerProcessProxy_async] couldn't register server %s process proxy:%s.",
            id.c_str(), toString(ex).c_str());
        cb->ice_exception(Ice::ServerNotFoundException());
    }
}
                                        ...
                                        const Ice::ObjectPrx& proxy)
{
    ...
    do
    {
        ...
        catch(const Ice::Exception& ex)
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "couldn't register adapter %s endpoints:\n %s.",
                    adapterId.c_str(), toString(ex).c_str());
            throw Ice::AdapterNotFoundException();
        }
        ...
        else
        {
            ...
            if(!session)
            {
                ...
                LOG(CSystemLog::LOG_ERROR, "couldn't register adapter %s endpoints with master:\n no session established with the master.",
                    adapterId.c_str());
                throw Ice::AdapterNotFoundException();
            }
