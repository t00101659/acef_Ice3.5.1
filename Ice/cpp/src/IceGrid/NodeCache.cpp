...
NodeEntry::setSession(const NodeSessionIPtr& session)
{
        ...
        while(_session)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    LOG(CSystemLog::LOG_ERROR, "[NodeEntry::setSession] Node:%s[%s] is active state.",
                        _name.c_str(), session->getNode()->ice_toString().c_str());
    ...
    if(session)
    {
        ...
        LOG(CSystemLog::LOG_DEBUG, "[NodeEntry::setSession] Node:%s[%s] is up.",
            _name.c_str(), session->getNode()->ice_toString().c_str());
    }
    else
    {
        ...
        LOG(CSystemLog::LOG_DEBUG, "[NodeEntry::setSession] Node:%s is down.",
            _name.c_str());
    }
}
