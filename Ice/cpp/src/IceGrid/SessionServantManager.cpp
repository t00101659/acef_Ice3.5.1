...
SessionServantManager::addSession(const Ice::ObjectPtr& session, const Ice::ConnectionPtr& con, const string& category)
{
    ...
    LOG(CSystemLog::LOG_SYSTEM, "[SessionServantManager::addSession] add session:%p, connection:%s,category:%s.",
        session.get(), con->toString().c_str(), category.c_str());
...
{
    LOG(CSystemLog::LOG_DEBUG, "[SessionServantManager::removeSession] remove session:%p.",
        session.get());
    Lock sync(*this);
