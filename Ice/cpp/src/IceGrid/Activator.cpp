                    ...
                    const ServerIPtr& server)
{
    ...
    LOG(CSystemLog::LOG_DEBUG, "[Activator::activate] process:%s exePath:%s pwdPath:%s.",
        name.c_str(), exePath.c_str(), pwdPath.c_str());
    if(_deactivating)
    {
        LOG(CSystemLog::LOG_ERROR, "The node is being shutdown");
    ...
    if(path.empty())
    {
        LOG(CSystemLog::LOG_ERROR, "The server executable path is empty.");
    ...
    if(!IceUtilInternal::isAbsolutePath(path))
    {
        if(path.find('/') == string::npos)
        {
            ...
            if(SearchPathW(NULL, IceUtil::stringToWstring(path).c_str(), ext.c_str(), _MAX_PATH, absbuf, &fPart) == 0)
            {
                ...
                LOG(CSystemLog::LOG_ERROR, "couldn't find %s executable", path.c_str());
    ...
    if(!pwd.empty())
    {
        ...
        if(_wfullpath(absbuf, IceUtil::stringToWstring(pwd).c_str(), _MAX_PATH) == NULL)
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "The server working directory path %s can't be converted into an absolute path.",
                pwd.c_str());
	...
	wstring wCmdStr(cmdbuf);
	string cmdStr = IceUtil::wstringToString(wCmdStr);

	OUTPUT_LOG_A(0, "icegridnode CreateProcess: ret: " << b << " cmdbuf:" << cmdStr << " dirPtr: " << dir << " pid: " << pi.dwProcessId << " tid: " << pi.dwThreadId);

    LOG(CSystemLog::LOG_SYSTEM, "[Activator::activate] CreateProcess: ret:%d,cmdbuf:%s,pid:%d,tid:%d",
        b, cmdStr.c_str(), pi.dwProcessId, pi.dwThreadId);
    free(cmdbuf);
...
Activator::deactivate(const string& name, const Ice::ProcessPrx& process)
{
    ...
    if(process)
    {
        ...
        LOG(CSystemLog::LOG_SYSTEM, "[Activator::deactivate] deactivating %s using process proxy.",
            name.c_str());
...
//		sendSignal(name, SIGTERM);
    ...
    LOG(CSystemLog::LOG_DEBUG, "[Activator::deactivate] no process proxy, deactivating %s using signal.",
        name.c_str());
...
Activator::sendSignal(const string& name, int signal)
{
    ...
    if(pid == 0)
    {
        ...
        // Server is already deactivated.
        //
        LOG(CSystemLog::LOG_ERROR, "[Activator::sendSignal] Server:%s is already deactivated",
            name.c_str());
        return;
    }
    ...
    else if(signal == SIGKILL)
    {
        ...
        LOG(CSystemLog::LOG_SYSTEM, "[Activator::sendSignal] terminated server %s, pid:%d",
            name.c_str(), pid);
    }
    else
    {
        LOG(CSystemLog::LOG_ERROR, "[Activator::sendSignal] signal not supported on Windows");
        throw BadSignalException("signal not supported on Windows");
    }
#else
    LOG(CSystemLog::LOG_SYSTEM, "[Activator::sendSignal] send kill signal:%d to server:%s, pid:%d.", 
        signal, name.c_str(), pid);
...
Activator::start()
{
    ...
    //
    LOG(CSystemLog::LOG_DEBUG, "[Activator::start] Create and start the termination listener thread");
...
Activator::destroy()
{
    ...
    for(map<string, Process>::iterator p = processes.begin(); p != processes.end(); ++p)
    {
        ...
        catch(const Ice::LocalException& ex)
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "[Activator::destroy] unexpected exception raised by server %s stop:%s.",
                p->first.c_str(), ex.what());
        }
    }
...
Activator::runTerminationListener()
{
    while(true)
    {
        ...
        catch(const Exception& ex)
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "[Activator::runTerminationListener] exception in process termination listener:%s",
                ex.what());
        }
        catch(...)
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "[Activator::runTerminationListener] unknown exception in process termination listener");
        }
    }
}
...
Activator::terminationListener()
{
    ...
    while(true)
    {
        ...
        for(vector<Process>::const_iterator p = terminated.begin(); p != terminated.end(); ++p)
        {
            ...
            LOG(CSystemLog::LOG_DEBUG, "[Activator::terminationListener] detected termination of server %s ",
                p->server->getId().c_str());
            ...
            catch(const Ice::LocalException& ex)
            {
                ...
                LOG(CSystemLog::LOG_ERROR, "[Activator::terminationListener] unexpected exception raised by server %s termination:%s",
                    p->server->getId().c_str(), ex.what());
            }
