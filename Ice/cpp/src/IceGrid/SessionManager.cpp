...
SessionManager::SessionManager(const Ice::CommunicatorPtr& communicator) : _communicator(communicator)
{
    if(communicator->getDefaultLocator())
    {
        ...
        LOG(CSystemLog::LOG_SYSTEM, "[SessionManager::SessionManager] getDefaultLocator:%s.", prx->ice_toString().c_str());
        //
        ...
        LOG(CSystemLog::LOG_SYSTEM, "[SessionManager::SessionManager] InternalRegistryPrx:%s.", 
            _master->ice_toString().c_str());
        Ice::EndpointSeq seq = _master->ice_getEndpoints();
        for (unsigned int i = 0; i < seq.size(); i++)
        {
            LOG(CSystemLog::LOG_DEBUG, "[SessionManager::SessionManager] endpoint:%s.", seq[i]->toString().c_str());
        }
    }
}
