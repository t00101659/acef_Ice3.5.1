...
AdminSessionI::_register(const SessionServantManagerPtr& servantManager, const Ice::ConnectionPtr& con)
{
    ...
    LOG(CSystemLog::LOG_DEBUG, "[AdminSessionI::_register] add Session:%s,id:%s.",
        session->ice_toString().c_str(), this->getId().c_str());
    _admin = AdminPrx::uncheckedCast(_servantManager->add(new AdminI(_database, _registry, this), this));

    LOG(CSystemLog::LOG_DEBUG, "[AdminSessionI::_register] _admin proxy:%s,id:%s.",
        _admin->ice_toString().c_str(), this->getId().c_str());
    return session;
}
...
AdminSessionI::destroyImpl(bool shutdown)
{
    ...
    LOG(CSystemLog::LOG_SYSTEM, "[AdminSessionI::destroyImpl] remove session:%s", this->getId().c_str());
    try
    {
        _database->unlock(this);
    }
