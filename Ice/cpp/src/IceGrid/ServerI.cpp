...
ServerI::start(ServerActivation activation, const AMD_Server_startPtr& amdCB)
{
    ServerCommandPtr command;
    {
        ...
        {
            LOG(CSystemLog::LOG_ERROR, "[ServerI::start] The server is disabled.");
        ...
        {
            LOG(CSystemLog::LOG_ERROR, "[ServerI::start] The server[%s] activation timed out.",
                _id.c_str());
        ...
        else if(_state == Active)
        {
            ...
            LOG(CSystemLog::LOG_DEBUG, "[ServerI::start] The server[%s] is already active.",
                _id.c_str());
            throw ServerStartException(_id, "The server is already active.");
        }
        else if(_state == Destroying)
        {
            LOG(CSystemLog::LOG_ERROR, "[ServerI::start] The server[%s] is being destroyed.",
                _id.c_str());
...
ServerI::activate()
{
    ...
    try
    {
        ...
        LOG(CSystemLog::LOG_DEBUG, "[ServerI::activate] Start server:%s,exe:%s,pwd:%s", desc->id.c_str(),
            desc->exe.c_str(), desc->pwd.c_str());
#ifndef _WIN32
    ...
    catch(const Ice::Exception& ex)
    {
        ...
        os << ex;
        failure = os.str();
        LOG(CSystemLog::LOG_ERROR, "[ServerI::activate] activation failed for server %s:%s",
            _id.c_str(), ex.what());
    }
...
ServerI::kill()
{
    ...
    catch(const Ice::SyscallException& ex)
    {
        ...
        out << ex;
        LOG(CSystemLog::LOG_ERROR, "[ServerI::kill] deactivation failed for server %s:%s", _id.c_str(), ex.what());
...
#endif
            os << (msg.empty() ? string(".") : ":\n" + msg);
            LOG(CSystemLog::LOG_DEBUG, "[ServerI::terminated] The server terminated unexpectedly %s", 
                os.str().c_str());
