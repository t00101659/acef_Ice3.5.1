    ...
    _destroy(false)
{
    ...
    try
    {
        ...
        LOG(CSystemLog::LOG_SYSTEM, "[NodeSessionI::NodeSessionI] publish nodesession proxy :%s for node:%s node proxy:%s",
            _proxy->ice_toString().c_str(), info->name.c_str(), _node->ice_toString().c_str());
    }
...
NodeSessionI::destroyImpl(bool shutdown)
{
    ...
    LOG(CSystemLog::LOG_ERROR, "[NodeSessionI::destroyImpl] Node[%s] is down,registry[%d].",
        _node->ice_toString().c_str(), shutdown);
    ServerEntrySeq servers = _database->getNode(_info->name)->getServers();
