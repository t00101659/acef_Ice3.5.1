...
RegistryI::startImpl()
{
    LOG(CSystemLog::LOG_SYSTEM, "[RegistryI::startImpl] Enter startImpl.");
    ...
    if(properties->getProperty("IceGrid.Registry.Client.Endpoints").empty())
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[RegistryI::startImpl] property `IceGrid.Registry.Client.Endpoints' is not set.");
        return false;
    }
    ...
    if(properties->getProperty("IceGrid.Registry.Server.Endpoints").empty())
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[RegistryI::startImpl] property `IceGrid.Registry.Server.Endpoints' is not set.");
        return false;
    }
    ...
    if(properties->getProperty("IceGrid.Registry.Internal.Endpoints").empty())
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[RegistryI::startImpl] property `IceGrid.Registry.Internal.Endpoints' is not set.");
        return false;
    }
    ...
    if(!properties->getProperty("IceGrid.Registry.SessionManager.Endpoints").empty())
    {
        if(!_nowarn)
        {
            ...
            {
                //std::cout << " (with Glacier2 filters disabled)";
            }
        }
    }
    ...
    if(!_initFromReplica.empty() && (_initFromReplica == _replicaName || (_master && _initFromReplica == "Master")))
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[RegistryI::startImpl] invalid --initdb-from-replica option: identical replica.");
        return false; 
    }
    ...
    if(!_master && properties->getProperty("Ice.Default.Locator").empty())
    {
        if(properties->getProperty("Ice.Default.Locator").empty())
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "[RegistryI::startImpl] property `Ice.Default.Locator' is not set.");
            return false;
        }
    }
    ...
    //
    LOG(CSystemLog::LOG_SYSTEM, "[RegistryI::startImpl] Create the registry database.");
    ...
    if(!plugin)
    {
        ...
        LOG(CSystemLog::LOG_SYSTEM, "[RegistryI::startImpl] no database plugin configured with `Ice.Plugin.DB' or plugin is not a database plugin.");
        return false;
    }
    if(!plugin->initDB())
    {
        LOG(CSystemLog::LOG_ERROR, "[RegistryI::startImpl] Failed to init DB.");
        return false;
    }
	...
	ObjectAdapterPtr registryAdapter;
    LOG(CSystemLog::LOG_SYSTEM, "[RegistryI::startImpl] Create the internal registry object adapter.");
	try
	{
		registryAdapter = _communicator->createObjectAdapter("IceGrid.Registry.Internal");
		registryAdapter->activate();
	}
	catch (const Ice::Exception& ex)
	{
		OUTPUT_LOG_A(1, "The Adapter IceGrid.Registry.Internal Throw Exception: " << ex.what());
        LOG(CSystemLog::LOG_ERROR, "[RegistryI::startImpl] The Adapter IceGrid.Registry.Internal Throw Exception:%s.",
            ex.what());
		throw;
	}
    ...
    //
    LOG(CSystemLog::LOG_SYSTEM, "[RegistryI::startImpl] Create the internal IceStorm service.");
    ...
    if(!_initFromReplica.empty())
    {
     ...
     //std::cout << " run 11" <<std::endl;
        ...
        if(!proxy)
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "[RegistryI::startImpl] couldn't find replica %s, to" \
                "initialize the database (specify the replica endpoints in the endpoints of" \
                "the `Ice.Default.Locator' proxy property to allow finding the replica).", 
                _initFromReplica.c_str());
            return false;
        }
     //std::cout << " run 12" <<std::endl;
        ...
        catch(const Ice::OperationNotExistException&)
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "[RegistryI::startImpl] couldn't initialize database from replica %s:" \
                "replica doesn't support this functionality (IceGrid < 3.5.1).", _initFromReplica.c_str());
            return false;
        }
        catch(const Ice::Exception& ex)
        {
            ...
            out << "couldn't initialize database from replica `" << _initFromReplica << "':\n";
            out << ex;
            LOG(CSystemLog::LOG_ERROR, "[RegistryI::startImpl] couldn't initialize database from replica %s :%s.",
                _initFromReplica.c_str(), ex.what());
            return false;
        }
    }
	...
	{
		OUTPUT_LOG_A(1, "Create Adapter IceGrid.Registry.Server Exception: " << ex.what());
        LOG(CSystemLog::LOG_ERROR, "[RegistryI::startImpl] Create Adapter IceGrid.Registry.Server Exception:%s.",
            ex.what());
		throw;
	}
	...
	{
		OUTPUT_LOG_A(1, "Create Adapter IceGrid.Registry.Client Exception: " << ex.what());
        LOG(CSystemLog::LOG_ERROR, "[RegistryI::startImpl] Create Adapter IceGrid.Registry.Client Exception:%s.",
            ex.what());
		throw;
	}
    ...
    _servantManager = new SessionServantManager(_clientAdapter, _instanceName, false, getServerAdminCategory(), 
    ...
    LOG(CSystemLog::LOG_SYSTEM, "[RegistryI::startImpl] Leave startImpl.");
    return true;
}
...
RegistryI::createAdminSession(const string& user, const string& password, const Current& current)
{
	...
	OUTPUT_LOG_A(0, "RegistryI::createAdminSession, current id:" << current.id.name << " current.con ptr:" << current.con.get());
    LOG(CSystemLog::LOG_SYSTEM, "[RegistryI::createAdminSession] current id:%s.", current.id.name.c_str());
