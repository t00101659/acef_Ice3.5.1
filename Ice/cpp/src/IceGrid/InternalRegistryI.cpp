                                ...
                                const Ice::Current& current)
{
    ...
    try
    {
        ...
        LOG(CSystemLog::LOG_SYSTEM, "[InternalRegistryI::registerNode] Register success:%s,timeout:%d,proxy:%s.", 
            info->name.c_str(), _nodeSessionTimeout, node->ice_toString().c_str());
        return session->getProxy();
    }
    catch(const Ice::ObjectAdapterDeactivatedException&)
    {
        LOG(CSystemLog::LOG_ERROR, "[InternalRegistryI::registerNode] ObjectAdapterDeactivatedException.");
