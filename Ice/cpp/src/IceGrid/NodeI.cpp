        ...
        while(true)
        {
            ...
            try
            {
                ...
                if(!server)
                {
                    ...
                    LOG(CSystemLog::LOG_DEBUG, "[NodeI::loadServer_async] ServerPrx:%s, _serversDir:%s, descriptor->id:%s,_waitTime:%d", 
                        proxy->ice_toString().c_str(), _serversDir.c_str(), descriptor->id.c_str(), _waitTime);
