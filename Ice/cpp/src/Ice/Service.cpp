...
bool g_bIfIsRegistry = false;	//check Registry
...
Ice::Service::main(int& argc, char* argv[], const InitializationData& initializationData)
{
...
#ifdef _WIN32
	//read file and set config IP
	std::string strPath;
	char szPath[512] = { 0 };
	GetModuleFileNameA(NULL, szPath, 511);
	strPath = szPath;
	...
	{
		NetType = strPath.substr(pos + 1);
	}
	string FileName = strPath + "\\" + NetType + "ServiceDog.ini";
	char szIpList[511 + 1] = { 0 };
	GetPrivateProfileStringA("Settings", "IpList", "", szIpList, 511, FileName.c_str());
	string strCfgIp = szIpList;
	CRS::SplitIPString(strCfgIp, ";");//save the IP list
#endif
    int logFlag = initData.properties->getPropertyAsIntWithDefault("CRS.ICE.Log.Flag", 0);
	if (0 != logFlag)
	{
        CRS::g_logMaxSize = initData.properties->getPropertyAsIntWithDefault("CRS.ICE.Log.LogMaxSize", 10);
        CRS::g_logNetType = initData.properties->getProperty("CRS.ICE.Log.NetType");
        CRS::g_logLevel = initData.properties->getPropertyAsIntWithDefault("CRS.ICE.Log.LogLevel", 4);

        string strTmpPath = initData.properties->getProperty("Ice.Config");
        string srServiceName = strTmpPath.substr(strTmpPath.find_last_of("/\\")+1);
            
        string strLogPath = initData.properties->getProperty("CRS.ICE.Log.LogPath");
        if (!strLogPath.empty())
        {
#ifdef _WIN32
            strLogPath += "\\";
        ...
        {
            strTmpPath = strTmpPath.substr(0, strTmpPath.find_last_of("/\\")+1);
        ...
        string strNodeName = "none";
        if (srServiceName == "config.master")
        {
			g_bIfIsRegistry = true;
            strNodeName = "master";
        }
        else if (srServiceName == "config.node1")
        {
            strNodeName = "node_node1";
        }
        else if (srServiceName == "dispatchservice.node1")
        {
            strNodeName = "node_dis1";
        }
        else if (srServiceName == "locationservice.node1")
        {
            strNodeName = "node_loc1";
        }
        else if (srServiceName == "interworkingservice.node1")
        {
            strNodeName = "node_inter1";
        }
        else if (srServiceName == "config.glacier2")
        {
            strNodeName = "glacier2";
        }
        else
        {
        }
        printf("strLogPath =%s.\n", strLogPath.c_str());
        CRS::InitChaosLogFile(strNodeName, strLogPath);
    }
#ifdef _WIN32
    //
...
Ice::Service::run(int& argc, char* argv[], const InitializationData& initData)
{
    ...
    try
    {
        ...
        //
        LOG(CSystemLog::LOG_SYSTEM, "[Service::run] Initialize the communicator.");
        ...
        // Start the service.
        //
        LOG(CSystemLog::LOG_SYSTEM, "[Service::run] Start the service.");
        if(start(argc, argv, status))
        {
			//write file to notify registry status
			//1.get NetType and service name
			string strPath;
#ifdef _WIN32
			char szPath[260 * 2] = {0};
			...
			if(g_bIfIsRegistry)
			{
				string filename = strPath + "\\checkRegistry.txt";
				fstream fs(filename.c_str(), std::ios::out);
				if(!fs.is_open())
				{
					 LOG(CSystemLog::LOG_SYSTEM, "[Service::run] open file checkRegistry.txt failed.");
				}
				else
				{
					fs << 1;
            ...
            //
            LOG(CSystemLog::LOG_SYSTEM, "[Service::run] Wait for service shutdown.");
            waitForShutdown();
    ...
    catch(const std::exception& ex)
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[Service::run] service caught unhandled exception:%s.", ex.what());
    }
    catch(const std::string& msg)
    {
        ServiceError err(this);
        err << "service caught unhandled exception:\n" << msg;
        LOG(CSystemLog::LOG_ERROR, "[Service::run] service caught unhandled exception:%s.", msg.c_str());
    }
    catch(const char* msg)
    {
        ServiceError err(this);
        err << "service caught unhandled exception:\n" << msg;
        LOG(CSystemLog::LOG_ERROR, "[Service::run] service caught unhandled exception:%s.", msg);
    }
    catch(...)
    {
        error("service caught unhandled C++ exception");
        LOG(CSystemLog::LOG_ERROR, "[Service::run] service caught unhandled C++ exception.");
    }
