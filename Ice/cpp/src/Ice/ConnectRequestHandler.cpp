...
ConnectRequestHandler::getConnection(bool waitInit)
{
    if(waitInit)
    {
        ...
        {
            /*
            LOG(CSystemLog::LOG_DEBUG, "[ConnectRequestHandler::getConnection] Wait for the connection establishment" \
                "to complete or fail [%d].", waitInit);*/
            wait();
        }
    }
    ...
    if(_exception.get())
    {
        LOG(CSystemLog::LOG_ERROR, "[ConnectRequestHandler::getConnection] %s exception:%s.",
            getReference()->toString().c_str(), _exception.get()->what());
    ...
    else
    {
        ...
        /*LOG(CSystemLog::LOG_DEBUG, "[ConnectRequestHandler::getConnection] connected [%s][%s][%d].", 
            _connection->getInfo()->adapterName.c_str(),  _connection->getInfo()->connectionId.c_str(), waitInit);*/
        return _connection;
    }
}
...
ConnectRequestHandler::setConnection(const Ice::ConnectionIPtr& connection, bool compress)
{
	...
	OUTPUT_LOG_B(0, "!reference->getRouterInfo() return ri-P:" << ri.get() << " add proxy:[" << _proxy->ice_toString() << "]");
    //LOG(CSystemLog::LOG_DEBUG, "[ConnectRequestHandler::setConnection] !reference->getRouterInfo() return ri-P:0X%p,add proxy:[%s].",
    //    ri.get(), _proxy->ice_toString().c_str());
	...
	OUTPUT_LOG_B(0, "setConnection phase 1");
    //LOG(CSystemLog::LOG_DEBUG, "[ConnectRequestHandler::setConnection] setConnection phase 1");
    //
    // We can now send the queued requests.
    //
    flushRequests();
    //LOG(CSystemLog::LOG_DEBUG, "[ConnectRequestHandler::setConnection] valid connection:%s,_requests:%d.", 
    //    _connection?_connection->toString().c_str():"", _requests.size());
}
...
ConnectRequestHandler::setException(const Ice::LocalException& ex)
{
    ...
    LOG(CSystemLog::LOG_ERROR, "[ConnectRequestHandler::setException] connection:%s ex:%s.", 
        _connection?_connection->toString().c_str():"", ex.what());
    notifyAll();
}
...
ConnectRequestHandler::flushRequests()
{
    ...
    catch(const LocalExceptionWrapper& ex)
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[ConnectRequestHandler::flushRequests] ex:%s.",
            ex.get()->what());
    }
