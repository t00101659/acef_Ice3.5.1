                                         ...
                                         InvocationObserver& observer)
{
    ...
    try
    {
		std::string strLog="__handleException";
		//char buf[1024]={0};
		//sprintf(buf,"__handleException cnt:%d [%s]",cnt,ex.what());
		//strLog=buf;
		//WriteLog(strLog);
	//	strLog="__handleException " +ex->operation;
			//	WriteLog(strLog);
        ...
        try
        {
			...
			/*if(one)
			{
				strLog="__handleException ObjectNotExistException [" +one->operation +"]";
				WriteLog(strLog);
				if(_reference->getRouterInfo() && one->operation.find("ice_add_proxy")==0)
				{
					strLog="__handleException ice_add_proxy";
					WriteLog(strLog);
...
IceDelegateM::Ice::Object::setup(const ReferencePtr& ref, const ::Ice::ObjectPrx& proxy, bool async)
{
	...
	{
		OUTPUT_LOG_B(0, "ref->getRouterInfo() return valid ptr, reference-P:" << ref.get() << " proxy:[" << proxy->ice_toString() << "]  async:" << async);
        //LOG(CSystemLog::LOG_HIDE, "[IceDelegateM::Ice::Object::setup] ref->getRouterInfo() return valid ptr, reference-P:0X%p proxy:[%s] async:%d.", ref.get(), proxy->ice_toString().c_str(), async);
	}
	else
	{
		OUTPUT_LOG_B(0, "ref->getRouterInfo() return NULL ptr, reference-P:" << ref.get() << " proxy:[" << proxy->ice_toString() << "]  async:" << async);
        //LOG(CSystemLog::LOG_HIDE, "[IceDelegateM::Ice::Object::setup] ref->getRouterInfo() return NULL ptr, reference-P:0X%p  proxy:[%s] async:%d.", ref.get(), proxy->ice_toString().c_str(), async);
	}
