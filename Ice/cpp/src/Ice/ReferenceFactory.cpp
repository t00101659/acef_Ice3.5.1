...
{
    /*
    LOG(CSystemLog::LOG_HIDE, "[ReferenceFactory::create] Create reference %s:%s.",
        str.c_str(), propertyPrefix.c_str());*/
    if(str.empty())
    {
        return 0;
    }
    ...
    {
        /*
        LOG(CSystemLog::LOG_HIDE, "[ReferenceFactory::create] Create a well known reference[idstring:%s,facet:%s,mode:%d,\
            secure:%d,propertyPrefix:%s].", idstr.c_str(), facet.c_str(), mode, secure, propertyPrefix.c_str());*/
    ...
    switch(s[beg])
    {
        case ':':
        {
            ...
            /*
            LOG(CSystemLog::LOG_HIDE, "[ReferenceFactory::create] Create a direct proxy reference[idstring:%s,facet:%s,mode:%d,"\
            "secure:%d,propertyPrefix:%s].", idstr.c_str(), facet.c_str(), mode, secure, propertyPrefix.c_str());            */
                                      ...
                                      const string& propertyPrefix)
{
	...
	OUTPUT_LOG_B(0, "Get routerInfo from defaultRouter, routerInfo-P:" << routerInfo.get() << ", defaultRouter-P:" << _defaultLocator.get());
    ...
    if(!propertyPrefix.empty())
    {
        ...
        if(router)
        {
            ...
            else
            {
				...
				OUTPUT_LOG_B(0, "Get routerInfo from propertyRouter, routerInfo-P:" << routerInfo.get() << ", router-P:" << router.get());
            }
        }
