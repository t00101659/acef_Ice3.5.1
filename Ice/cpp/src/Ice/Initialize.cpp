...
//#ifdef OS_WIN
	...
	if (0 != logFlag)
	{
		...
		CRS::InitChaosLogFile("corelib", strLogPath);// duplicate call is safe
	}
#ifdef OS_WIN
    ...
    LOG(CSystemLog::LOG_SYSTEM, "[Ice::initialize] Communicator pointer:0x%p logFlag:%d.", communicatorI, logFlag);

    printf("[Ice::initialize] Communicator pointer:0x%p logFlag:%d.\n", communicatorI, logFlag);
...
Ice::initialize(const InitializationData& initData, Int version)
{
	...
	OUTPUT_LOG_A(1, "Ice::initialize 2, Communicator pointer:" << reinterpret_cast<unsigned int>(communicatorI));
