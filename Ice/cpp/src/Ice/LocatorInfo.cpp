...
public:
    ...
    virtual void send(bool async)
    {
        try
        {
            //LOG(CSystemLog::LOG_INFO, "[ObjectRequest::send] begin_findObjectById :%s, async:%d.", 
            //    _ref->getInstance()->identityToString(_ref->getIdentity()).c_str(), async);
            if(async)
            {
...
public:
    ...
    virtual void send(bool async)
    {
        try
        {
            //LOG(CSystemLog::LOG_INFO, "[AdapterRequest::send] begin_findAdapterById :%s, async:%d.", 
            //    _ref->getAdapterId().c_str(), async);
            if(async)
            {
...
IceInternal::LocatorTable::getAdapterEndpoints(const string& adapter, int ttl, vector<EndpointIPtr>& endpoints)
{
    ...
    {
        //LOG(CSystemLog::LOG_ERROR, "[LocatorTable::getAdapterEndpoints] ttl = 0.");
        return false;
    }
...
IceInternal::LocatorTable::getObjectReference(const Identity& id, int ttl, ReferencePtr& ref)
{
    if(ttl == 0) // No locator cache
    {
        //LOG(CSystemLog::LOG_ERROR, "[LocatorTable::getObjectReference] ttl = 0.");
        return false;
    }
    ...
    {
        ref = p->second.second;
        //LOG(CSystemLog::LOG_INFO, "[LocatorTable::getObjectReference] getObjectReference %s.", 
        //    ref->getInstance()->identityToString(id).c_str());
        return checkTTL(p->second.first, ttl);
    }
    return false;
}
...
{
    //LOG(CSystemLog::LOG_INFO, "[LocatorTable::addObjectReference] addObjectReference %s.", 
    //    ref->getInstance()->identityToString(id).c_str());
    IceUtil::Mutex::Lock sync(*this);
...
IceInternal::LocatorTable::removeObjectReference(const Identity& id)
{
    ...
    //LOG(CSystemLog::LOG_INFO, "[LocatorTable::removeObjectReference] removeObjectReference %s.", 
    //    ref->getInstance()->identityToString(id).c_str());
    _objectMap.erase(p);
    return ref;
}
...
{
    if (_ref && _ref->getInstance()->traceLevels()->location >= 1)
    {
        LOG(CSystemLog::LOG_ERROR, "[Request::response] response proxy:%s from AdapterId:%s,id:%s.", 
            proxy?proxy->ice_toString().c_str():"",_ref->getAdapterId().c_str(), 
            _ref->getInstance()->identityToString(_ref->getIdentity()).c_str());
    }
...
{
    LOG(CSystemLog::LOG_ERROR, "[Request::exception] exception :%s from AdapterId:%s,id:%s.", ex.what(), 
        _ref->getAdapterId().c_str(), _ref->getInstance()->identityToString(_ref->getIdentity()).c_str());
...
IceInternal::LocatorInfo::getEndpoints(const ReferencePtr& ref, const ReferencePtr& wellKnownRef, int ttl, bool& cached)
{
    ...
    if(ref->getInstance()->traceLevels()->location >= 1)
    {
        ...
        if(!ref->isWellKnown())
        {
            LOG(CSystemLog::LOG_INFO, "[LocatorInfo::getEndpoints] Found endpoints in locator table adapter = %s endpoints = %s", 
                ref->getAdapterId().c_str(), o.str().c_str());
        }
        else
        {
            LOG(CSystemLog::LOG_INFO, "[LocatorInfo::getEndpoints] Found endpoints in locator table object = %s endpoints = %s", 
                ref->getInstance()->identityToString(ref->getIdentity()).c_str(), o.str().c_str());
        }
...
{
    //LOG(CSystemLog::LOG_INFO, "[LocatorInfo::getEndpoints] Enter getEndpoints %s", 
    //    ref->getInstance()->identityToString(ref->getIdentity()).c_str());
    assert(ref->isIndirect());
    vector<EndpointIPtr> endpoints;
    if(!ref->isWellKnown())
    {
    ...
    {
        //LOG(CSystemLog::LOG_INFO, "[LocatorInfo::getEndpoints] Enter getEndpoints %s,ttl:%d", 
        //    ref->getInstance()->identityToString(ref->getIdentity()).c_str(), ttl);
        assert(ref->isIndirect());
        ReferencePtr r;
        if(!_table->getObjectReference(ref->getIdentity(), ttl, r))
        {
        ...
        else if(!r->isWellKnown())
        {
            if (ref->getInstance()->traceLevels()->location >= 1)
            {
                LOG(CSystemLog::LOG_INFO, "[LocatorInfo::getEndpoints] r:%s:%s,ref:%s:%s.",
                    r->getAdapterId().c_str(), r->getInstance()->identityToString(r->getIdentity()).c_str(),
                    ref->getAdapterId().c_str(), ref->getInstance()->identityToString(ref->getIdentity()).c_str());
            }
    ...
    //LOG(CSystemLog::LOG_INFO, "[LocatorInfo::getEndpoints] Leave getEndpoints %s:%s", 
    //    ref->getInstance()->identityToString(ref->getIdentity()).c_str(), o.str().c_str());
}
...
IceInternal::LocatorInfo::getAdapterRequest(const ReferencePtr& ref)
{
    if(ref->getInstance()->traceLevels()->location >= 1)
    {
        LOG(CSystemLog::LOG_INFO, "[LocatorInfo::getAdapterRequest] Searching for adapter by id:%s",
            ref->getAdapterId().c_str());
        //Trace out(ref->getInstance()->initializationData().logger, ref->getInstance()->traceLevels()->locationCat);
...
IceInternal::LocatorInfo::getObjectRequest(const ReferencePtr& ref)
{
    if(ref->getInstance()->traceLevels()->location >= 1)
    {
        LOG(CSystemLog::LOG_INFO, "[LocatorInfo::getObjectRequest] Searching for object by id:%s",
            ref->getInstance()->identityToString(ref->getIdentity()).c_str());
        //Trace out(ref->getInstance()->initializationData().logger, ref->getInstance()->traceLevels()->locationCat);
                                        ...
                                        bool notRegistered)
{
    if(!proxy || proxy->__reference()->isIndirect())
    {
        ...
        {
            //LOG(CSystemLog::LOG_INFO, "[LocatorInfo::finishRequest] removeObjectReference :%s.", 
            //    ref->getInstance()->identityToString((*q)->getIdentity()).c_str());
