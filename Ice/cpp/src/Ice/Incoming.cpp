...
IceInternal::Incoming::invoke(const ServantManagerPtr& servantManager, BasicStream* stream)
{
        ...
        if(!_servant)
        {
            ...
            if(_locator)
            {
                ...
                catch(const UserException& ex)
                {
                    ...
                    _observer.userException();
                    LOG(CSystemLog::LOG_ERROR, "[Incoming::invoke] User exception:%s [Identity=%s][operation=%s][facet=%s][requestId=%d].",
                        ex.what(), _current.id.name.c_str(), _current.operation.c_str(),
                        _current.facet.c_str(), _current.requestId);
                    if(_response)
                    {
                        _os.write(replyUserException);
    ...
    static volatile int g_serverthreadcount = 0;
    try
    {
        g_serverthreadcount++;
        if(_servant)
        {
            ...
            //
			IceUtil::Int64 dwtick = IceUtil::Time::now().toMilliSeconds();
            
            //LOG(CSystemLog::LOG_DEBUG, "[Incoming::invoke] Dispatch to service[Identity=%s][operation=%s][facet=%s][requestId=%d].",
            //    _current.id.name.c_str(), _current.operation.c_str(),
            //        _current.facet.c_str(), _current.requestId);
			...
			dwtick = IceUtil::Time::now().toMilliSeconds()-dwtick;
			if (dwtick >= 100)
			{
                LOG(CSystemLog::LOG_SYSTEM, "[Incoming::invoke] RPC[Identity=%s][operation=%s][facet=%s][requestId=%d]" \
                    " is Invoked spend time[%d] ms overtime 100ms with con:%s.", _current.id.name.c_str(), _current.operation.c_str(),
                    _current.facet.c_str(), _current.requestId, static_cast<int>(dwtick), _connection->toString().c_str());
                LOG(CSystemLog::LOG_SYSTEM, "[Incoming::invoke] Server BusyThread Num is %d.", g_serverthreadcount);
			}
            /*
            else
            {
                LOG(CSystemLog::LOG_HIDE, "[Incoming::invoke] RPC[Identity=%s][operation=%s][facet=%s][requestId=%d]" \
                    " is Invoked spend time for %lldms.", _current.id.name.c_str(), _current.operation.c_str(),
                    _current.facet.c_str(), _current.requestId, dwtick);
                LOG(CSystemLog::LOG_HIDE, "[Incoming::invoke] Server BusyThread Num is %d.", g_serverthreadcount);
            }*/
			g_serverthreadcount--;
        ...
        else
        {
            ...
            {
                LOG(CSystemLog::LOG_ERROR, "[Incoming::invoke] Object NotExist Exception [Identity=%s][operation=%s][facet=%s][requestId=%d] exception.",
                    _current.id.name.c_str(), _current.operation.c_str(), _current.facet.c_str(), _current.requestId);
    ...
    catch(const std::exception& ex)
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[Incoming::invoke] __handleException [Identity=%s][operation=%s][facet=%s][requestId=%d] exception:%s.",
                    _current.id.name.c_str(), _current.operation.c_str(), _current.facet.c_str(), _current.requestId, ex.what());
        __handleException(ex);
        return;
    }
    catch(...)
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[Incoming::invoke] __handleException [Identity=%s][operation=%s][facet=%s][requestId=%d] exception.",
                    _current.id.name.c_str(), _current.operation.c_str(), _current.facet.c_str(), _current.requestId);
        __handleException();
        return;
    }
