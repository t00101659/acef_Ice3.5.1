...
extern IceInternal::outgoingoperationhandler g_operationhandler;
...
{
    LOG(CSystemLog::LOG_SYSTEM, "[ConnectionI::close] Forced close connection:%d.", force);
    IceUtil::Monitor<IceUtil::Mutex>::Lock sync(*this);
...
Ice::ConnectionI::monitor(const IceUtil::Time& now, const ACMConfig& acm)
{
    ...
    if(acm.close != CloseOff && now >= (_acmLastActivity + acm.timeout))
    {
           ...
           (acm.close != CloseOnIdle && (!_requests.empty() || !_asyncRequests.empty())))
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "[ConnectionI::monitor] we didn't receive a heartbeat in the last period[%d] secs." 
                " the connection[%s:%d] is closed.",
                static_cast<int>(acm.timeout.toSeconds()), _transceiver->toString().c_str(), getNativeInfo()->fd());
        }
        else if(acm.close != CloseOnInvocation &&
                _dispatchCount == 0 && _requests.empty() && _asyncRequests.empty())
        {
            //
            // The connection is idle, close it.
            //
            setState(StateClosing, ConnectionTimeoutException(__FILE__, __LINE__));
            LOG(CSystemLog::LOG_SYSTEM, "[ConnectionI::monitor] connection[%s:%d] is idle for %d secs to be closed.",
                _transceiver->toString().c_str(), getNativeInfo()->fd() ,static_cast<int>(acm.timeout.toSeconds()));
        }
    }
}
...
Ice::ConnectionI::sendRequest(Outgoing* out, bool compress, bool response)
{
    ...
    if(response)
    {
        ...
        //LOG(CSystemLog::LOG_HIDE, "[Ice::ConnectionI::sendRequest] requestId=%d.", requestId);
        g_operationhandler.SetRequestId(out, requestId);
        //
    ...
    {
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::sendRequest] catch %s to send message, and close connection.", ex.what());
...
Ice::ConnectionI::sendAsyncRequest(const OutgoingAsyncPtr& out, bool compress, bool response)
{
    ...
    //
    _transceiver->checkSendSize(*os, _instance->messageSizeMax());
    //LOG(CSystemLog::LOG_DEBUG, "[ConnectionI::sendAsyncRequest] connection:%s,requestId=%d,response:%d.", 
    //    toString().c_str(), _nextRequestId,response);
    Int requestId = 0;
    if(response)
    {
        //
        ...
        //LOG(CSystemLog::LOG_DEBUG, "[Ice::ConnectionI::sendAsyncRequest] requestId=%d.", requestId);
        //
        // Fill in the request ID.
        //
        const Byte* p = reinterpret_cast<const Byte*>(&requestId);
    ...
    {
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::sendAsyncRequest] catch %s to send message, and close connection.", ex.what());
        setState(StateClosed, ex);
        assert(_exception.get());
        _exception->ice_throw();
    }
...
Ice::ConnectionI::prepareBatchRequest(BasicStream* os)
{
    ...
    if(_batchStream.b.empty())
    {
        ...
        {
            LOG(CSystemLog::LOG_ERROR, "[ConnectionI::prepareBatchRequest] catch %s to send message, and close connection.", ex.what());
            setState(StateClosed, ex);
            ex.ice_throw();
        }
    }
...
Ice::ConnectionI::finishBatchRequest(BasicStream* os, bool compress)
{
    try
    {
        ...
        if(flush)
        {
            ...
            {
                LOG(CSystemLog::LOG_ERROR, "[ConnectionI::finishBatchRequest] catch %s to send message, and close connection.", ex.what());
                setState(StateClosed, ex);
                assert(_exception.get());
                _exception->ice_throw();
            }
...
Ice::ConnectionI::flushBatchRequests(BatchOutgoing* out)
{
    ...
    {
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::flushBatchRequests] catch %s to send message, and close connection.", ex.what());
        setState(StateClosed, ex);
        assert(_exception.get());
        _exception->ice_throw();
    }
...
Ice::ConnectionI::flushAsyncBatchRequests(const BatchOutgoingAsyncPtr& outAsync)
{
    ...
    {
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::flushAsyncBatchRequests] catch %s to send message, and close connection.", ex.what());
        setState(StateClosed, ex);
        assert(_exception.get());
        _exception->ice_throw();
    }
        ...
        if(_state >= StateClosed)
        {
            if(callback)
            {
                ...
                public:
                    ...
                    CallbackWorkItem(const InstancePtr& instance, const ConnectionIPtr& connection, const ConnectionCallbackPtr& callback) :
                    ...
                    virtual void run()
                    {
                        ...
                        LOG(CSystemLog::LOG_DEBUG, "[CallbackWorkItem::run] run closeCallback:%p for %s.", 
                            _callback.get(), _connection->toString().c_str());
                    }
                ...
                _threadPool->execute(new CallbackWorkItem(_instance, this, callback));
            }
        }
        else
        {
            _callback = callback;
            LOG(CSystemLog::LOG_DEBUG, "[ConnectionI::setCallback] _callback:%p for connection:%s.", 
                _callback.get(), this->toString().c_str());
        }
    }
}
...
Ice::ConnectionI::closeCallback(const ConnectionCallbackPtr& callback)
{
    try
    {
        ...
        LOG(CSystemLog::LOG_SYSTEM, "[ConnectionI::closeCallback] close connection:%s.",
            _transceiver->toString().c_str());
    }
    catch(const std::exception& ex)
    {
        Error out(_instance->initializationData().logger);
        out << "connection callback exception:\n" << ex << '\n' << _desc;
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::closeCallback] connection callback exception:%s:%s.",
            ex.what(), _desc.c_str());
    }
    catch(...)
    {
        Error out(_instance->initializationData().logger);
        out << "connection callback exception:\nunknown c++ exception" << '\n' << _desc;
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::closeCallback] connection callback exception:unknown c++ exception:%s.",
            _desc.c_str());
    }
}
...
Ice::ConnectionI::sendResponse(BasicStream* os, Byte compressFlag)
{
    ...
    try
    {
        ...
        {
            LOG(CSystemLog::LOG_DEBUG, "[ConnectionI::sendResponse] initiateShutdown %s.", toString().c_str());
            initiateShutdown();
        }
    ...
    {
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::sendResponse] exception:%s to close connection.", ex.what());
        setState(StateClosed, ex);
    }
}
...
Ice::ConnectionI::sendNoResponse()
{
    ...
    {
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::sendNoResponse] catch %s to close connection.", ex.what());
        setState(StateClosed, ex);
    }
}
...
Ice::ConnectionI::startAsync(SocketOperation operation)
{
    ...
    {
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::startAsync] catch %s to close connection:%s.", 
            ex.what(), toString().c_str());
        setState(StateClosed, ex);
        return false;
    }
    return true;
...
Ice::ConnectionI::finishAsync(SocketOperation operation)
{
    ...
    {
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::finishAsync] catch %s to close connection.", ex.what());
        setState(StateClosed, ex);
    }
...
Ice::ConnectionI::message(ThreadPoolCurrent& current)
{
...

    {
        ...
        try
        {
            ...
            if(dispatchCount == 0)
            {
                //LOG(CSystemLog::LOG_DEBUG, "this connection:%s Nothing to dispatch we're done.",
                //    toString().c_str());
        ...
        {
            LOG(CSystemLog::LOG_ERROR, "[ConnectionI::message] catch %s to close connection.", ex.what());
            setState(StateClosed, ex);
            return;
        }
        catch(const LocalException& ex)
    ...
    if(_dispatcher)
    {
        ...
        catch(const std::exception& ex)
        {
            ...
            LOG(CSystemLog::LOG_WARNING, "[ConnectionI::message] dispatch exception :%s.", ex.what());
        }
        catch(...)
        {
            if(_instance->initializationData().properties->getPropertyAsIntWithDefault("Ice.Warn.Dispatch", 1) > 1)
            ...
            LOG(CSystemLog::LOG_WARNING, "[ConnectionI::message] dispatch exception :unknown c++ exception.");
        }
    }
    else
    {
...
#endif
            //LOG(CSystemLog::LOG_DEBUG, "[ConnectionI::dispatch] heartbeat %s.", _desc.c_str());
        }
        catch(const std::exception& ex)
        {
            Error out(_instance->initializationData().logger);
            out << "connection callback exception:\n" << ex << '\n' << _desc;
            LOG(CSystemLog::LOG_ERROR, "[ConnectionI::dispatch] connection callback exception:%s:%s.",
                ex.what(), _desc.c_str());
        }
        catch(...)
        {
            Error out(_instance->initializationData().logger);
            out << "connection callback exception:\nunknown c++ exception" << '\n' << _desc;
            LOG(CSystemLog::LOG_ERROR, "[ConnectionI::dispatch] connection callback exception:unknown c++ exception:%s.",
                _desc.c_str());
        }
        ++dispatchedCount;
    }
    ...
    if(dispatchedCount > 0)
    {
        ...
        if(_dispatchCount == 0)
        {
            ...
            if(_state == StateClosing)
            {
                ...
                {
                    LOG(CSystemLog::LOG_ERROR, "[ConnectionI::dispatch] catch %s to close connection.", ex.what());
                    setState(StateClosed, ex);
                }
            }
...
Ice::ConnectionI::finish()
{
    ...
    LOG(CSystemLog::LOG_DEBUG, "[ConnectionI::finish] connection:%s.", _desc.c_str());
    //
...
Ice::ConnectionI::timedOut()
{
    ...
    if(_state <= StateNotValidated)
    {
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::timedOut] Timeout while establishing a connection from %s" \
            "[adapter:%s].", _desc.c_str(), _adapter->getName().c_str());
        setState(StateClosed, ConnectTimeoutException(__FILE__, __LINE__));
    }
    else if(_state < StateClosing)
    {
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::timedOut] Timeout while sending or receiving data from %s" \
            "[adapter:%s].", _desc.c_str(), _adapter->getName().c_str());
        setState(StateClosed, TimeoutException(__FILE__, __LINE__));
    }
    else if(_state == StateClosing)
    {
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::timedOut] Timeout while closing a connection from %s" \
            "[adapter:%s].", _desc.c_str(), _adapter->getName().c_str());
...
{
    LOG(CSystemLog::LOG_ERROR, "[ConnectionI::exception] catch %s to close connection.", ex.what());
    IceUtil::Monitor<IceUtil::Mutex>::Lock sync(*this);
    setState(StateClosed, ex);
}
...
Ice::ConnectionI::invokeException(const LocalException& ex, int invokeNum)
{
    ...
    //
    LOG(CSystemLog::LOG_ERROR, "[ConnectionI::invokeException] catch %s to close connection.", ex.what());
    IceUtil::Monitor<IceUtil::Mutex>::Lock sync(*this);
    setState(StateClosed, ex);
    ...
    if(!_exception.get())
    {
        ...
        if(_warn && _validated)
        {
                 ...
                 (dynamic_cast<const ConnectionLostException*>(_exception.get()) && _state == StateClosing)))
            {
                ...
                LOG(CSystemLog::LOG_WARNING, "[ConnectionI::setState] connection exception:%s,%s.",
                    (*_exception.get()).what(), _desc.c_str());
            }
        }
    }
...
Ice::ConnectionI::setState(State state)
{
    ...
    catch(const Ice::LocalException& ex)
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::setState] unexpected connection exception:%s,%s.",
            ex.what(), _desc.c_str());
    }
    ...
    if(_state == StateClosing && _dispatchCount == 0)
    {
        ...
        {
            LOG(CSystemLog::LOG_ERROR, "[ConnectionI::setState] catch %s to close connection.", ex.what());
            setState(StateClosed, ex);
        }
    }
}
...
Ice::ConnectionI::heartbeat()
{
    ...
    if(!_endpoint->datagram())
    {
        ...
        try
        {
            OutgoingMessage message(&os, false);
            sendMessage(message);
            //LOG(CSystemLog::LOG_DEBUG, "[ConnectionI::heartbeat] send heartbeat message to svr connection:%s.",
            //    toString().c_str());
        }
        catch(const LocalException& ex)
        {
...
Ice::ConnectionI::sendNextMessage(vector<SentCallback>& callbacks)
{
    ...
    {
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::sendNextMessage] catch %s to close connection.", ex.what());
        setState(StateClosed, ex);
        return;
    }
                               ...
                               int& dispatchCount)
{
    ...
    try
    {
        ...
        switch(messageType)
        {
            case closeConnectionMsg:
            {
                ...
                {
                    LOG(CSystemLog::LOG_ERROR, "[ConnectionI::parseMessage] Recv closeConnectionMsg to close connection.");
            ...
            case requestMsg:
            {
                if(_state == StateClosing)
                {
                    trace("received request during closing\n(ignored by server, client will retry)", stream, _logger,
                          _traceLevels);
                    LOG(CSystemLog::LOG_DEBUG, "[ConnectionI::parseMessage] received request during closing(ignored by server, client will retry).");
                }
                else
                {
                    traceRecv(stream, _logger, _traceLevels);
    ...
    {
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::parseMessage] Local exception:%s to close connection.",
            ex.what());
        if(_endpoint->datagram())
        {
            if(_warn)
                            ...
                            const ServantManagerPtr& servantManager, const ObjectAdapterPtr& adapter)
{
    ...
    {
        LOG(CSystemLog::LOG_ERROR, "[ConnectionI::invokeAll] Fatal invocation exception:%s.",
            ex.what());
...
ConnectionI::reap()
{
    LOG(CSystemLog::LOG_DEBUG, "[ConnectionI::reap] reap connection:%s.", toString().c_str());
    if(_monitor)
    {
        _monitor->reap(this);
    }
