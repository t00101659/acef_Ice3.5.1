...
IceInternal::outgoingoperationhandler g_operationhandler;
...
{ 
    ...
    //LOG(CSystemLog::LOG_DEBUG, "[Outgoing::Outgoing] Client rpc request[ice_toString=%s][operation=%s].",
    //    _handler->getReference()->toString().c_str(), operation.c_str());
	g_operationhandler.Addoperation(this,operation);
    ...
    try
    {
        ...
        g_operationhandler.SetId(this, const_cast<Ice::Identity&>(_handler->getReference()->getIdentity()));
        g_operationhandler.SetFacet(this, const_cast<string&>(_handler->getReference()->getFacet()));
        g_operationhandler.SetAdapterId(this, _handler->getReference()->getAdapterId());
        g_operationhandler.SetConnectionId(this, _handler->getReference()->getConnectionId());
        g_operationhandler.SetLocatorInfo(this, _handler->getReference()->getLocatorInfo());
        g_operationhandler.SetRouterInfo(this, _handler->getReference()->getRouterInfo());
...
Outgoing::~Outgoing()
{
	g_operationhandler.Removeoperation(this);
...
IceInternal::Outgoing::invoke()
{
    ...
    switch(_handler->getReference()->getMode())
    {
        case Reference::ModeTwoway:
        {
            ...
            g_operationhandler.SetStartTick(this, dwtick);
            ...
            if(timedOut)
            {
                ...
                // object.
                //
                LOG(CSystemLog::LOG_ERROR, "[Outgoing::invoke] RPC[ice_toString=%s][operation=%s][requestId=%d] is timedOut for %d ms with conn:%s.",
                    _handler->getReference()->toString().c_str(), 
                    g_operationhandler.Getoperation(this).c_str(), 
                    g_operationhandler.GetRequestId(this),
                    connection->timeout(),
			...
			if (dwtick >= 100 || (_handler->getReference()->getInstance()->traceLevels()->location >= 1))
			{
                LOG(CSystemLog::LOG_SYSTEM, "[Outgoing::invoke] RPC[ice_toString=%s][operation=%s][requestId=%d][size=%d] is invoked" \
                    " spend time[%d]ms with con:%s.", _handler->getReference()->toString().c_str(), 
                    g_operationhandler.Getoperation(this).c_str(), g_operationhandler.GetRequestId(this), 
                    static_cast<int>(os()->b.size()),
                    static_cast<int>(dwtick),
                    connection->toString().c_str());
			}
