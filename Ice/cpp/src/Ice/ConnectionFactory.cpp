...
Ice::EndpointSelectionType selType, bool& compress, bool bmanulchangetimeout)
{
    assert(!endpts.empty());
	...
	if (!bmanulchangetimeout)
	{
		endpoints = applyOverrides(endpts);
	}
	else
	{
		endpoints = endpts;
...
IceInternal::OutgoingConnectionFactory::create(const vector<EndpointIPtr>& endpts, bool hasMore,
                                               ...
                                               const CreateConnectionCallbackPtr& callback,bool bmanulchangetimeout)
{
    assert(!endpts.empty());
...
IceInternal::OutgoingConnectionFactory::ConnectCallback::nextEndpoint()
{
    ...
    {
        exception(ex);
        LOG(CSystemLog::LOG_ERROR, "[ConnectCallback::nextEndpoint] endpoint:[%s:%d] Ex:%s.",
            (*_endpointsIter)->GetHost().c_str(),(*_endpointsIter)->GetPort(), ex.what());
    }
}
...
IceInternal::OutgoingConnectionFactory::ConnectCallback::getConnection()
{
    try
    {
        ...
        if(!connection)
        {
            ...
            if (_connectors.size() >= 1)
                LOG(CSystemLog::LOG_ERROR, "[ConnectCallback::getConnection] null connection[local:%s:%d,remote:%s].",
                    _connectors[0].endpoint?_connectors[0].endpoint->GetHost().c_str():"", 
                    _connectors[0].endpoint?_connectors[0].endpoint->GetPort():0,
                    _connectors[0].endpoint?_connectors[0].connector->toString().c_str():"");
            else 
                LOG(CSystemLog::LOG_ERROR, "[ConnectCallback::getConnection] null connection.");
            return;
        }
    ...
    catch(const Ice::LocalException& ex)
    {
        _callback->setException(ex);
        _factory->decPendingConnectCount(); // Must be called last.
        LOG(CSystemLog::LOG_ERROR, "[ConnectCallback::getConnection] exception:%s.", ex.what());
    }
}
...
IceInternal::IncomingConnectionFactory::startAsync(SocketOperation)
{
    ...
    catch(const Ice::LocalException& ex)
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[IncomingConnectionFactory::startAsync] Can't accept connections: %s, " \
            "last error:%d.", _acceptor->toString().c_str(),
            GetLastError());
...
IceInternal::IncomingConnectionFactory::finishAsync(SocketOperation)
{
    ...
    catch(const LocalException& ex)
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[IncomingConnectionFactory::finishAsync] Couldn't accept connection:%s,error:%d.",
            _acceptor->toString().c_str(), ex.what());
        return false;
    }    
...
IceInternal::IncomingConnectionFactory::message(ThreadPoolCurrent& current)
{
...

    {
        ...
        catch(const SocketException& ex)
        {
            if(noMoreFds(ex.error))
            {
                ...
                LOG(CSystemLog::LOG_ERROR, "[IncomingConnectionFactory::message]fatal error:%d, " \
                    "can't accept more connections:%s.", ex.error, 
                    _acceptor->toString().c_str());
        ...
        catch(const LocalException& ex)
        {
            ...
            LOG(CSystemLog::LOG_WARNING, "[IncomingConnectionFactory::message] connection exception:%s from %s,",
                ex.what(), _acceptor->toString().c_str());
            return;
        }
        ...
        try
        {
			...
			//Ice::IPConnectionInfoPtr infoPtr = dynamic_cast<Ice::IPConnectionInfo*>(connection.get());

			//OUTPUT_LOG_A(0, "Accept a New Connection, ptr:" << connection.get());
            LOG(CSystemLog::LOG_INFO, "[IncomingConnectionFactory::message] Accept a New Connection, ptr:0x%p.",
                connection.get());

			Ice::IPConnectionInfoPtr infoPtr = Ice::IPConnectionInfoPtr::dynamicCast(connection->getInfo());
			if (infoPtr)
			{
				//OUTPUT_LOG_A(0, "Connection info, remote:[" << infoPtr->remoteAddress << ":" << infoPtr->remotePort << "] local:[" << infoPtr->localAddress << ":" << infoPtr->localPort << "]");
                LOG(CSystemLog::LOG_INFO, "[IncomingConnectionFactory::message] Connection info, remote:[%s:%d]" \
                    "local:[%s:%d].", infoPtr->remoteAddress.c_str(), infoPtr->remotePort, infoPtr->localAddress.c_str(),
                    infoPtr->localPort);
			}
        ...
        catch(const LocalException& ex)
        {
            ...
            LOG(CSystemLog::LOG_INFO, "[IncomingConnectionFactory::message] connection[%s] exception,error:%s.", 
                _acceptor->toString().c_str(), ex.what());
            return;
        }
    ...
    LOG(CSystemLog::LOG_INFO, "[IncomingConnectionFactory::message] size of _connections:%d for %s.", 
        (int)_connections.size(), _name.c_str());
    assert(connection);
    connection->start(this);
}
...
{
    const_cast<string&>(_name) = oaName;
    ...
    try
    {
        ...
        if(_transceiver)
        {
            ConnectionIPtr connection = new ConnectionI(_adapter->getCommunicator(), _instance, _monitor, _transceiver,
                                                        0, _endpoint, _adapter);
            connection->start(0);  
            LOG(CSystemLog::LOG_INFO, "[IncomingConnectionFactory::initialize] _transceiver is not null.");
            _connections.insert(connection);
        }
        else
        {
    ...
    {
        LOG(CSystemLog::LOG_ERROR, "[IncomingConnectionFactory::initialize] Failed to create tcpAcceptor, error: %s.",
            ex.what());
        if(_transceiver)
        {
            try
    ...
    switch(state)
    {
        case StateActive:
        {
            ...
            if(_acceptor)
            {
                ...
                LOG(CSystemLog::LOG_INFO, "[IncomingConnectionFactory::setState] Accepting %s connections at %s.",
                    _endpoint->protocol().c_str(), _acceptor->toString().c_str());
