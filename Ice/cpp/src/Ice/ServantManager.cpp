...
IceInternal::ServantManager::addServant(const ObjectPtr& object, const Identity& ident, const string& facet)
{
    ...
    else
    {
        if(p->second.find(facet) != p->second.end())
        {
            ...
            LOG(CSystemLog::LOG_WARNING, "[IceInternal::ServantManager::addServant] Servant[%s] has been already" \
                " registered.", ex.id.c_str());
            throw ex;
        }
    }
	...
	{
        string id = _instance->identityToString(ident);
        if(!facet.empty())
        {
            string fs = nativeToUTF8(_instance->initializationData().stringConverter, facet);
            id += " -f " + IceUtilInternal::escapeString(fs, "");
        }
        LOG(CSystemLog::LOG_SYSTEM, "[IceInternal::ServantManager::addServant] Add servant[%s] to ServantMap" \
            "[totalsize=%d] with adapter[%s].", id.c_str(), (int)_servantMapMap.size(),
            _adapterName.c_str());
	}
}
...
IceInternal::ServantManager::addDefaultServant(const ObjectPtr& object, const string& category)
{
    ...
    {
        LOG(CSystemLog::LOG_SYSTEM, "[IceInternal::ServantManager::addDefaultServant] Add default servant[category=%s] " \
            " to DefaultServantMap[totalsize=%d] with adapter[%s].", category.c_str(), 
            (int)_defaultServantMap.size(), _adapterName.c_str());
	}
}
...
IceInternal::ServantManager::removeServant(const Identity& ident, const string& facet)
{
    ...
    if(p == _servantMapMap.end() || (q = p->second.find(facet)) == p->second.end())
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[IceInternal::ServantManager::removeServant] Servant[%s] is not registered in adapter[%s].",
            ex.id.c_str(), _adapterName.c_str());
        throw ex;
    }
...

    {
        ...
        LOG(CSystemLog::LOG_SYSTEM, "[IceInternal::ServantManager::removeServant] Remove servant[%s] from ServantMap" \
            "[remain=%d] with adapter[%s].", id.c_str(), (int)_servantMapMap.size(), _adapterName.c_str());
    }
...
IceInternal::ServantManager::findServant(const Identity& ident, const string& facet) const
{
    ...
    if(p == servantMapMap.end() || (q = p->second.find(facet)) == p->second.end())
    {
        ...
        if(p == _defaultServantMap.end())
        {
            ...
            else
            {
                ...
                LOG(CSystemLog::LOG_HIDE, "[IceInternal::ServantManager::findServant] Get servant[%s] from DefaultServantMap" \
                    "[totalsize=%d] with adapter[%s].", id.c_str(), (int)_defaultServantMap.size(), _adapterName.c_str());
                return p->second;
            }
        }
        else
    ...
    else
    {
        _servantMapMapHint = p;
        {
            ...
            /*LOG(CSystemLog::LOG_HIDE, "[IceInternal::ServantManager::findServant] Get servant[%s] from ServantMap" \
                "[totalsize=%d] with adapter[%s].", id.c_str(), _servantMapMap.size(), _adapterName.c_str());*/
        }
        return q->second;
    }
}
