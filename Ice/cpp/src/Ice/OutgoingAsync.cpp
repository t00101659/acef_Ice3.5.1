...
IceInternal::OutgoingAsync::__prepare(const std::string& operation, OperationMode mode, const Context* context)
{
    ...
    //LOG(CSystemLog::LOG_DEBUG, "[OutgoingAsync::__prepare] Client rpc request[ice_toString=%s][operation=%s].",
    //    ref->toString().c_str(), operation.c_str());
}
...
IceInternal::OutgoingAsync::__sent(Ice::ConnectionI* connection)
{
    ...
    //LOG(CSystemLog::LOG_DEBUG, "[OutgoingAsync::__sent] Client rpc request[ice_toString=%s][connection=%s][timeout:%d].",
    //    _proxy->ice_toString().c_str(), connection->toString().c_str(), connection->timeout());
    _monitor.notifyAll();
...
IceInternal::OutgoingAsync::__finished()
{
	...
	static volatile int g_amithreadcount=0;
	g_amithreadcount++;
	...
	if (dwtick >= 100 || (_instance->traceLevels()->location >= 1))
	{
        LOG(CSystemLog::LOG_SYSTEM, "[IceInternal::OutgoingAsync::__finished] OutgoingAsync RPC[ice_toString=%s][operation=%s][size=%d] is " \
            "invoked spend time[%d]ms overtime 100ms.", _proxy->ice_toString().c_str(), _operation.c_str(), 
            static_cast<int>(_os.b.size()),
            static_cast<int>(dwtick));
        LOG(CSystemLog::LOG_SYSTEM, "[IceInternal::OutgoingAsync::__finished] g_amithreadcount is %d.", g_amithreadcount);
...
IceInternal::OutgoingAsync::__send(bool synchronous)
{
    while(true)
    {
        ...
        try
        {
            ...
            //LOG(CSystemLog::LOG_DEBUG, "[OutgoingAsync::__send] sendAsyncRequest rpc request[ice_toString=%s][status:%d][connection=%s][timeout:%d].",
            //    _proxy->ice_toString().c_str(), status,_timerTaskConnection?_timerTaskConnection->toString().c_str():"", 
            //    _timerTaskConnection?_timerTaskConnection->timeout():0);
...
IceInternal::OutgoingAsync::runTimerTask() // Implementation of TimerTask::runTimerTask()
{
    ...
    if(connection)
    {
        LOG(CSystemLog::LOG_ERROR, "[OutgoingAsync::runTimerTask] RPC[ice_toString=%s][operation=%s] [address:%s] timeOut for %d ms.", 
            _proxy->ice_toString().c_str(), _operation.c_str(), connection->toString().c_str(),connection->timeout());
