...
IceInternal::TcpEndpointI::expand() const
{
    ...
    else
    {
        for(vector<string>::const_iterator p = hosts.begin(); p != hosts.end(); ++p)
        {
            ...
            LOG(CSystemLog::LOG_INFO, "[TcpEndpointI::expand] Create TcpEndpointI[%s:%d,_timeout=%d,_connectionId=%s," \
                "_compress=%d] for addr:%s successfully.",
                p->c_str(), _port, _timeout, _connectionId.c_str(), _compress, _host.c_str());
        }
    }
    return endps;
}
