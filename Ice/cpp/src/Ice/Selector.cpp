...
Selector::setup(int sizeIO)
{
    ...
    if(_handle == NULL)
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[Selector::setup] Ice::SocketException:%d.", ex.error);
        throw ex;
    }
    LOG(CSystemLog::LOG_INFO, "[Selector::setup] Create IoCompletion Port[handle=%d][threadnum=%d] successfully.",
        _handle, sizeIO);
}
...
Selector::initialize(EventHandler* handler)
{
    ...
    if(CreateIoCompletionPort(socket, _handle, reinterpret_cast<ULONG_PTR>(handler), 0) == NULL)
    {
        Ice::SocketException ex(__FILE__, __LINE__);
        ex.error = GetLastError();
        LOG(CSystemLog::LOG_ERROR, "[Selector::initialize] Ice::SocketException:%d.", ex.error);
        throw ex;
    }
    handler->__incRef();
    LOG(CSystemLog::LOG_INFO, "[Selector::initialize] Enable IoCompletion Port[socket=%d] successfully.",
        _handle);
}
...
Selector::getNextHandler(SocketOperation& status, int timeout)
{
    ...
    if(!GetQueuedCompletionStatus(_handle, &count, &key, &ol, timeout > 0 ? timeout * 1000 : INFINITE))
    {
		...
		if (ol == 0)
		{
			if (err == WAIT_TIMEOUT)
			{
			    //hang up and waiting timeout return
				throw SelectorTimeoutException();
			}
			else
			{
                LOG(CSystemLog::LOG_ERROR, "[Selector::getNextHandler] Could not dequeue packet from completion port" \
                    ":%d.", err);
        ...
        handler = reinterpret_cast<EventHandler*>(key);
        LOG(CSystemLog::LOG_ERROR, "[Selector::getNextHandler] GetQueuedCompletionStatus[key=%s,count=%d] error:%d.", 
            handler?handler->toString().c_str():"", count, err);
        return handler;
    }
    ...
    handler = reinterpret_cast<EventHandler*>(key);
    /*
    LOG(CSystemLog::LOG_HIDE, "[Selector::getNextHandler] Slector GetQueuedCompletionStatus[count=%d,key=%s] success.",
        count, handler?handler->toString().c_str():"");*/
    return handler;
}
