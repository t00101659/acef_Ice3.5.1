...
{ 
    LOG(CSystemLog::LOG_SYSTEM, "CommunicatorI::shutdown");
...
{
	//CRS::InitChaosLogFile("corelib");

	//CRS::CTSelectorInit();// duplicate call is safe

    __setNoDelete(true);
    try
    {
        LOG(CSystemLog::LOG_SYSTEM, "[Ice::CommunicatorI::CommunicatorI] Enter CommunicatorI.");
        ...
        LOG(CSystemLog::LOG_SYSTEM, "[Ice::CommunicatorI::CommunicatorI] Leave CommunicatorI.");
    }
    catch(...)
    {
        destroy();
...
#ifdef OS_WIN
		string slctType = _instance->_initData.properties->getProperty("CRS.ICE.EndpointSelection");
		string messageSize = _instance->_initData.properties->getProperty("Ice.MessageSizeMax");

		LOG(CSystemLog::LOG_SYSTEM, "CRS.ICE.EndpointSelection = %s, Ice.MessageSizeMax = %s.", slctType.c_str(), messageSize.c_str());
		if ("CRSConnectionTest" == slctType)
		{
            LOG(CSystemLog::LOG_SYSTEM, "IOCP Connection Test Switch ON");
			CRS::CTSelectorInit();// duplicate call is safe
		}
		else
		{
            LOG(CSystemLog::LOG_SYSTEM, "IOCP Connection Test Switch OFF");
		}
#endif
