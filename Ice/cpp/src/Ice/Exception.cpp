...
Ice::SyscallException::ice_print(ostream& out) const
{
    ...
    if(error != 0)
    {
        out << ":\nsyscall exception: " << "error:[" << error << "]: " << IceUtilInternal::errorToString(error);
    }
}
...
{
    Exception::ice_print(out);
	out << ":\nsocket exception: " << "error:[" << error << "]: " << socketErrorToString(error);
}
...
Ice::FileException::ice_print(ostream& out) const
{
    ...
    {
		out << "error:[" << error << "]: " << IceUtilInternal::errorToString(error);
    }
    if(!path.empty())
    {
        out << "\npath: " << path;
...
{
    Exception::ice_print(out);
	out << ":\nconnect failed: " << "error:[" << error << "]: " << socketErrorToString(error);
}
...
{
    Exception::ice_print(out);
	out << ":\nconnection refused: " << "error:[" << error << "]: " << socketErrorToString(error);
}
...
Ice::ConnectionLostException::ice_print(ostream& out) const
{
    ...
    {
		out << "error:[" << error << "]: " << socketErrorToString(error);
    }
}
...
Ice::DNSException::ice_print(ostream& out) const
{
	...
	out << "error:[" << error << "]: " << socketErrorToString(error);
#else
	out << "error:[" << error << "]: " << errorToStringDNS(error);
#endif
    out << "\nhost: " << host;
}
