...
#   endif        
    ...
    LOG(CSystemLog::LOG_INFO, "[IceInternal::ThreadPool::ThreadPool] ThreadName:%s,_size=%d,_sizeMax=%d,_sizeWarn=%d,_sizeIO=%d," \
        "_threadIdleTime=%d,_serverIdleTime=%d.", _prefix.c_str(), _size, _sizeMax, _sizeWarn, _sizeIO, 
        _threadIdleTime, _serverIdleTime);
#ifdef ICE_USE_IOCP
    ...
    LOG(CSystemLog::LOG_INFO, "[IceInternal::ThreadPool::ThreadPool] ThreadName:%s,_stackSize=%d,_hasPriority=%d," \
        "_priority=%d.", _prefix.c_str(), (int)_stackSize, _hasPriority, _priority);
    ...
    {
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "[IceInternal::ThreadPool::ThreadPool] Cannot create thread for %s" \
                ",exception:%s.", _prefix.c_str(), ex.what());
        }
...
IceInternal::ThreadPool::run(const EventHandlerThreadPtr& thread)
{
    ...
    while(true)
    {
        ...
        catch(const SelectorTimeoutException&)
        {
            if(_sizeMax > 1)
            {
                ...
                else if(_inUse < static_cast<int>(_threads.size() - 1)) // If not the last idle thread, we can exit.
                {
                    ...
                    //LOG(CSystemLog::LOG_HIDE, "[ThreadPool::run] Thread is shrinking for %s, Current thread size:%d.",
                    //    _prefix.c_str(), _threads.size() - 1);
            ...
            {
                LOG(CSystemLog::LOG_ERROR, "[ThreadPool::run] SelectorTimeoutException.");
                Lock sync(*this);
                if(!_destroyed)
                {
        ...
        catch(const exception& ex)
        {
            Error out(_instance->initializationData().logger);
            out << "exception in `" << _prefix << "':\n" << ex << "\nevent handler: " << current._handler->toString();
            LOG(CSystemLog::LOG_ERROR, "[ThreadPool::run] exception in %s :%s, event handler:%s.", _prefix.c_str(),
                ex.what(), current._handler->toString().c_str());
        }
#ifdef ICE_OS_WINRT
        ...
        catch(...)
        {
            Error out(_instance->initializationData().logger);
            out << "exception in `" << _prefix << "':\nevent handler: " << current._handler->toString();
            LOG(CSystemLog::LOG_ERROR, "[ThreadPool::run] exception in %s :event handler:%s.", _prefix.c_str(),
                current._handler->toString().c_str());
        }
...
IceInternal::ThreadPool::ioCompleted(ThreadPoolCurrent& current)
{
    ...
    if(_sizeMax > 1)
    {
		...
		if (_inUse >= _sizeMax)
		{
            LOG(CSystemLog::LOG_ERROR, "[IceInternal::ThreadPool::ioCompleted] ThreadPoolSize reach limit size, _prefix:"\
                "%s [_sizeMax=%d][_inUse=%d][_threads.size()=%d][_sizeWarn=%d]", _prefix.c_str(), _sizeMax, _inUse, (int)_threads.size(),
                _sizeWarn);
		}
        ...
        if(_inUse == _sizeWarn)
        {
            ...
            LOG(CSystemLog::LOG_WARNING, "[IceInternal::ThreadPool::ioCompleted] Thread pool(%s) is running low on threads "\
                "[Size=%d][SizeMax=%d][SizeWarn=%d]", _prefix.c_str(), _size, _sizeMax, _sizeWarn);
        }
        ...
        {            
            ...
            if(_inUse < _sizeMax && _inUse == static_cast<int>(_threads.size()))
            {
                ...
                try
                {
                    //LOG(CSystemLog::LOG_DEBUG, "[ThreadPool::ioCompleted] Thread Pool Busy[inUse=%d,sizeMax=%d] for %s Create a new thread.",
                    //    _inUse, _sizeMax, _prefix.c_str());
                    EventHandlerThreadPtr thread = new EventHandlerThread(this, nextThreadId());
                    if(_hasPriority)
                    {
                        thread->start(_stackSize, _priority);
                ...
                catch(const IceUtil::Exception& ex)
                {
                    Error out(_instance->initializationData().logger);
                    out << "cannot create thread for `" << _prefix << "':\n" << ex;
                    LOG(CSystemLog::LOG_ERROR, "[ThreadPool::ioCompleted] Failed to create thread for %s,exception:%s.",
                        _prefix.c_str(), ex.what());
                }
            }
        }
    }
...
IceInternal::ThreadPool::startMessage(ThreadPoolCurrent& current)
{
    ...
    if(current._handler->_started & current.operation)
    {
        ...
        {
			// OUTPUT_LOG_A(1, "finishAsync return false, current.operation:" << current.operation);
...
IceInternal::ThreadPool::EventHandlerThread::run()
{
    ...
    catch(const exception& ex)
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[EventHandlerThread::run] exception in %s: %s",
            _pool->_prefix.c_str(), ex.what());
    }
    catch(...)
    {
        Error out(_pool->_instance->initializationData().logger);
        out << "unknown exception in `" << _pool->_prefix << "'"; 
        LOG(CSystemLog::LOG_ERROR, "[EventHandlerThread::run] unknown exception in %s.",
            _pool->_prefix.c_str());
    }
