...
Ice::ObjectAdapterI::waitForDeactivate()
{
    ...
    // finished.
    //
    LOG(CSystemLog::LOG_SYSTEM, "Now we wait until all incoming connection factories are finished.");
    for_each(incomingConnectionFactories.begin(), incomingConnectionFactories.end(),
...
Ice::ObjectAdapterI::isLocal(const ObjectPrx& proxy) const
{
    ...
    else if(ref->isIndirect())
    {
        ...
        // adapter id or replica group id.//增加判断是否有存在servant
        //
		return (ref->getAdapterId() == _id || ref->getAdapterId() == _replicaGroupId) && _servantManager->hasServant(ref->getIdentity());
    }
    else
    {
...
Ice::ObjectAdapterI::initialize(const RouterPrx& router)
{
    ...
    if(unknownProps.size() != 0 && properties->getPropertyAsIntWithDefault("Ice.Warn.UnknownProperties", 1) > 0)
    {
        ...
        for(unsigned int i = 0; i < unknownProps.size(); ++i)
        {
            out << "\n    " << unknownProps[i];
            strUnownProps += " ";
            strUnownProps += unknownProps[i];
        }
        LOG(CSystemLog::LOG_WARNING, "[Ice::ObjectAdapterI::initialize] Found unknown properties for object adapter[%s]" \
            " : %s.", _name.c_str(), strUnownProps.c_str());
    }
    ...
    try
    {
        ...
        if(router == 0 && noProps)
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "[Ice::ObjectAdapterI::initialize] Object adapter:%s requires configuration.",
                _name.c_str());
            throw ex;
        }
        ...
        LOG(CSystemLog::LOG_SYSTEM, "[Ice::ObjectAdapterI::initialize] _id=%s and _replicaGroupId=%s.",
            _id.c_str(), _replicaGroupId.c_str());
        ...
        catch(const ProxyParseException&)
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "[Ice::ObjectAdapterI::initialize] Invalid proxy options:%s for object adapter:%s",
                proxyOptions.c_str(), _name.c_str());
            throw ex;
        }
        ...
        LOG(CSystemLog::LOG_SYSTEM, "[Ice::ObjectAdapterI::initialize] %s.ThreadPool.Size=%d,%s.ThreadPool.SizeMax=" \
            "%d,%s.ThreadPool.ThreadPriority=%s.", _name.c_str(), threadPoolSize, _name.c_str(), threadPoolSizeMax,
            _name.c_str(), hasPriority?"true":"false");
        //
        ...
        if(router)
        {
            ...
            if(_routerInfo)
            {
                ...
                for (size_t i = 0; i != _routerEndpoints.size(); i++)
                {
                    LOG(CSystemLog::LOG_INFO, "[Ice::ObjectAdapterI::initialize] Router endpoint:%s.",
                        _routerEndpoints[i]->toString().c_str());
                }
                //
        ...
        else
        {
            ...
            for(vector<EndpointIPtr>::iterator p = endpoints.begin(); p != endpoints.end(); ++p)
            {
                ...
                IncomingConnectionFactoryPtr factory = new IncomingConnectionFactory(_instance, *p, this);
                 factory->initialize(_name + ".Endpoints");
                _incomingConnectionFactories.push_back(factory);
                LOG(CSystemLog::LOG_SYSTEM, "[ObjectAdapterI::initialize] Create IncomingConnectionFactory for %s.Endpoints" \
                    "[%s].", _name.c_str(), (*p)->toString().c_str());
            }
            ...
            if(endpoints.empty())
            {
                ...
                LOG(CSystemLog::LOG_INFO, "[ObjectAdapterI::initialize] created adapter:%s without endpoints.",
                    _name.c_str());
            }
    ...
    {
        LOG(CSystemLog::LOG_ERROR, "[ObjectAdapterI::initialize] Failed to init adapter, Throw Exception:%s.",
            ex.what());
        destroy();
        throw;
    }
    ...
    {
        LOG(CSystemLog::LOG_ERROR, "[ObjectAdapterI::initialize] Failed to init adapter.");
        destroy();
        throw;
    }
}
...
Ice::ObjectAdapterI::parseEndpoints(const string& endpts, bool oaEndpoints) const
{
    ...
    while(end < endpts.length())
    {
        ...
        if(endp == 0)
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "[ObjectAdapterI::parseEndpoints] invalid object adapter endpoint:%s.",
                s.c_str());
            throw ex;
        }
...
ObjectAdapterI::parsePublishedEndpoints()
{
    ...
    if(endpoints.empty())
    {
        ...
        {
            vector<EndpointIPtr> endps = _incomingConnectionFactories[i]->endpoint()->expand();//expand是在这里扩展0.0.0.0IP
            endpoints.insert(endpoints.end(), endps.begin(), endps.end());
        }
    }

	//czz:在这里统一做过滤处理
	vector<EndpointIPtr> After_endpoints;
#ifdef _WIN32    
	for (vector<EndpointIPtr>::iterator p = endpoints.begin(); p != endpoints.end(); ++p)
	{
		int flag = 0;
		for (unsigned int i = 0; i < g_FilterIps.size(); i++)
		{
			if (((*p)->GetHost()).substr(0, g_FilterIps[i].length()) == g_FilterIps[i])
			{
				flag = 1;
				break;
			}
		...
		if (!flag)
		{
			After_endpoints.push_back(*p);
		}
	}

	if (After_endpoints.size() == 0)//no IP left
	{
		LOG(CSystemLog::LOG_SYSTEM, "[ObjectAdapterI::parsePublishedEndpoints] all local IP was filted， no endpoints was left. So do not filt, use all local IP");
    ...
    if(_instance->traceLevels()->network >= 1)
    {
        ...
        out << "published After_endpoints for object adapter `" << getName() << "':\n";
        for(unsigned int i = 0; i < After_endpoints.size(); ++i)
        {
            if(i > 0)
            {
                out << ":";
            }
            out << After_endpoints[i]->toString();
        }
    }
    string strEndpoints;
    for(unsigned int i = 0; i < After_endpoints.size(); ++i)
    {
        ...
        strEndpoints += After_endpoints[i]->toString();
    }
    LOG(CSystemLog::LOG_SYSTEM, "[ObjectAdapterI::parsePublishedEndpoints] published After_endpoints for object adapter"\
        " %s :%s.", getName().c_str(), strEndpoints.c_str());

    return After_endpoints;
}
                                      ...
                                      bool registerProcess)
{
    ...
    if(!_id.empty())
    {
        ...
        catch(const AdapterNotFoundException&)
        {
            ...
            LOG(CSystemLog::LOG_ERROR, "couldn't update object adapter %s endpoints with the locator registry:\n" \
                "the object adapter is not known to the locator registry:%s.", _id.c_str(), _replicaGroupId.c_str());
