...
setTcpNoDelay(SOCKET fd)
{
    ...
    if(setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, (char*)&flag, int(sizeof(int))) == SOCKET_ERROR)
    {
        ...
        LOG(CSystemLog::LOG_ERROR, "[setTcpNoDelay] Failed to setsockopt TCP_NODELAY,error:%d.", ex.error);
        throw ex;
    }
}
...
setKeepAlive(SOCKET fd)
{
    ...
    if(setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, (char*)&flag, int(sizeof(int))) == SOCKET_ERROR)
    {
        ...
        SocketException ex(__FILE__, __LINE__);
        ex.error = getSocketErrno();
        LOG(CSystemLog::LOG_ERROR, "[setKeepAlive] Failed to setsockopt SO_KEEPALIVE,error:%d.", ex.error);
        throw ex;
    }
}
#endif
...
createSocketImpl(bool udp, int family)
{
    ...
    if(fd == INVALID_SOCKET)
    {
        SocketException ex(__FILE__, __LINE__);
        ex.error = getSocketErrno();
        LOG(CSystemLog::LOG_ERROR, "[createSocketImpl] Failed to create socket,error:%d.", ex.error);
        throw ex;
    }
...
IceInternal::createServerSocket(bool udp, const Address& addr, ProtocolSupport protocol)
{
    ...
    if(addr.saStorage.ss_family == AF_INET6 && protocol != EnableIPv4)
    {
        ...
        if(setsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&flag, int(sizeof(int))) == SOCKET_ERROR)
        {
            ...
            SocketException ex(__FILE__, __LINE__);
            ex.error = getSocketErrno();
            LOG(CSystemLog::LOG_ERROR, "[IceInternal::createServerSocket] Failed to create socket[IP=%s,Port=%d],error=%d.",
                inetAddrToString(addr).c_str(), getPort(addr), ex.error);
            throw ex;
        }
    }
    return fd;
...
IceInternal::closeSocket(SOCKET fd)
{
    ...
    if(closesocket(fd) == SOCKET_ERROR)
    {
        SocketException ex(__FILE__, __LINE__);
        ex.error = getSocketErrno();
        LOG(CSystemLog::LOG_ERROR, "[IceInternal::closeSocket] closesocket[socket=%d] exception:%d.", 
            fd, ex.error);
        throw ex;
    }
    WSASetLastError(error);
...
IceInternal::fdToLocalAddress(SOCKET fd, Address& addr)
{
    ...
    if(getsockname(fd, &addr.sa, &len) == SOCKET_ERROR)
    {
        ...
        SocketException ex(__FILE__, __LINE__);
        ex.error = getSocketErrno();
        LOG(CSystemLog::LOG_ERROR, "[IceInternal::fdToLocalAddress] Failed to getsockname,error:%d.", ex.error);
        throw ex;
    }
#else
...
IceInternal::setTcpBufSize(SOCKET fd, const Ice::PropertiesPtr& properties, const Ice::LoggerPtr& logger)
{
    ...
    if(sizeRequested > 0)
    {
        ...
        if(size > 0 && size < sizeRequested) // Warn if the size that was set is less than the requested size.
        {
            ...
            LOG(CSystemLog::LOG_WARNING, "[IceInternal::setTcpBufSize] TCP receive buffer size: requested size of %d" \
                " adjusted to %d.", sizeRequested, size);
        }
    }
    ...
    if(sizeRequested > 0)
    {
        ...
        if(size > 0 && size < sizeRequested) // Warn if the size that was set is less than the requested size.
        {
            ...
            LOG(CSystemLog::LOG_WARNING, "[IceInternal::setTcpBufSize] TCP send buffer size: requested size of  %d" \
                " adjusted to %d.", sizeRequested, size);
        }
    }
}
...
IceInternal::doBind(SOCKET fd, const Address& addr)
{
    ...
    if(::bind(fd, &addr.sa, size) == SOCKET_ERROR)
    {
        ...
        SocketException ex(__FILE__, __LINE__);
        ex.error = getSocketErrno();
        LOG(CSystemLog::LOG_ERROR, "[IceInternal::doBind] Socket is failed to bind local address:[IP=%s,Port=%d],error:%d.",
            inetAddrToString(addr).c_str(), getPort(addr), ex.error);
        throw ex;
    }
...
repeatListen:
    if(::listen(fd, backlog) == SOCKET_ERROR)
    {
        ...
        SocketException ex(__FILE__, __LINE__);
        ex.error = getSocketErrno();
        LOG(CSystemLog::LOG_ERROR, "[IceInternal::doListen] Failed to listen on socket,error:%d.",
            ex.error);
        throw ex;
    }
}
...
IceInternal::doConnectAsync(SOCKET fd, const Address& addr, AsyncInfo& info)
{
    ...
    if(::bind(fd, &bindAddr.sa, size) == SOCKET_ERROR)
    {
        SocketException ex(__FILE__, __LINE__);
        ex.error = getSocketErrno();
        LOG(CSystemLog::LOG_ERROR, "[IceInternal::doConnectAsync] Failed to bind addr:%s.",
                ...
                NULL) == SOCKET_ERROR)
    {
        SocketException ex(__FILE__, __LINE__);
        ex.error = getSocketErrno();
        LOG(CSystemLog::LOG_ERROR, "[IceInternal::doConnectAsync] Failed to get WSAID_CONNECTEX from WSAIoctl" \
            "[addr=%s].", fdToString(fd).c_str());
        throw ex;
    }        
    ...
    if(!ConnectEx(fd, &addr.sa, size, 0, 0, 0, &info))
    {
        if(!connectInProgress())
        {
            if(connectionRefused())
            {
                ConnectionRefusedException ex(__FILE__, __LINE__);
                ex.error = getSocketErrno();
                LOG(CSystemLog::LOG_ERROR, "[IceInternal::doConnectAsync] The attempt to connect was rejected[addr=%s]," \
                    "error=%d.", fdToString(fd).c_str(), ex.error);
                throw ex;
            }
            else if(connectFailed())
            {
                ConnectFailedException ex(__FILE__, __LINE__);
                ex.error = getSocketErrno();
                LOG(CSystemLog::LOG_ERROR, "[IceInternal::doConnectAsync] Connect Failed[addr=%s],error:%d.",
                    fdToString(fd).c_str(), ex.error);
                throw ex;
            }
            else
            {
                SocketException ex(__FILE__, __LINE__);
                ex.error = getSocketErrno();
                LOG(CSystemLog::LOG_ERROR, "[IceInternal::doConnectAsync] Connect ERROR[addr=%s],error:%d.",
                    fdToString(fd).c_str(), ex.error);
                throw ex;
            }
        }
    }
    LOG(CSystemLog::LOG_INFO, "[IceInternal::doConnectAsync] Establishes a connection to a specified socket[%d][addr=%s] " \
        "successfully.", fd, fdToString(fd).c_str());
}
...
IceInternal::doFinishConnectAsync(SOCKET fd, AsyncInfo& info)
{
    ...
    if(static_cast<int>(info.count) == SOCKET_ERROR)
    {
        ...
        if(connectionRefused())
        {
            ConnectionRefusedException ex(__FILE__, __LINE__);
            ex.error = getSocketErrno();
            LOG(CSystemLog::LOG_ERROR, "[IceInternal::doFinishConnectAsync] Connection refuse from addr=%s,ex:%d.",
                fdToString(fd).c_str(), ex.error);
            throw ex;
        }
        else if(connectFailed())
        {
            ConnectFailedException ex(__FILE__, __LINE__);
            ex.error = getSocketErrno();
            LOG(CSystemLog::LOG_ERROR, "[IceInternal::doFinishConnectAsync] Connection failed from addr=%s,ex:%d.",
                fdToString(fd).c_str(), ex.error);
            throw ex;
        }
        else
        {
            SocketException ex(__FILE__, __LINE__);
            ex.error = getSocketErrno();
            LOG(CSystemLog::LOG_ERROR, "[IceInternal::doFinishConnectAsync] Connection socket exception from addr=%s,ex:%d.",
                fdToString(fd).c_str(), ex.error);
            throw ex;
        }
    }
    ...
    if(setsockopt(fd, SOL_SOCKET, SO_UPDATE_CONNECT_CONTEXT, NULL, 0) == SOCKET_ERROR)
    {
        SocketException ex(__FILE__, __LINE__);
        ex.error = getSocketErrno();
        LOG(CSystemLog::LOG_ERROR, "[IceInternal::doFinishConnectAsync] Failed to setsockopt from addr=%s,ex:%d.",
                fdToString(fd).c_str(), ex.error);
        throw ex;
    }
    
    //
    // Prevent self connect (self connect happens on Windows when a client tries to connect to
    // a server which was just deactivated if the client socket re-uses the same ephemeral
    // port as the server).
    //
    Address localAddr;
    ...
    if(fdToRemoteAddress(fd, remoteAddr) && compareAddress(remoteAddr, localAddr) == 0)
    {
        ConnectionRefusedException ex(__FILE__, __LINE__);
        ex.error = 0; // No appropriate errno
        LOG(CSystemLog::LOG_ERROR, "[IceInternal::doFinishConnectAsync] Connection refuse addr=%s is the same.",
                fdToString(fd).c_str());
        throw ex;
    }
}
