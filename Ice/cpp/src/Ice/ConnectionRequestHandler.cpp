    ...
    RequestHandler(reference)
{
    ...
    if(ri)
    {
		OUTPUT_LOG_B(0, "reference->getRouterInfo() return valid ptr, ri-P:" << ri.get() << " add proxy:[" << proxy->ice_toString() << "]");

        ri->addProxy(proxy);
    }
	else
	{
		OUTPUT_LOG_B(0, "reference->getRouterInfo() return NULL ptr, ri-P:" << ri.get() << " add proxy:[" << proxy->ice_toString() << "]");
	}
}
