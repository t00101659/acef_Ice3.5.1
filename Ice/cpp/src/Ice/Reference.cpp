...
#include "Chaos_ConnectionTest.icpp.cpp"
using namespace CRS;
...
IceInternal::RoutableReference::RoutableReference(const InstancePtr& instance, 
    ...
    _timeout(-1),
	_flagCTConnectionTest(0)
{
	OUTPUT_LOG_B(0, "New RoutableReference, routerInfo-P:" << _routerInfo.get());
    
   /* LOG(CSystemLog::LOG_HIDE, "[RoutableReference::RoutableReference] New RoutableReference id=%s,facet=%s,mode=%d,secure=%d," \
        "_endpoints.size=%d,_adapterId=%s,_locatorInfo:0x%x,_routerInfo=0x%x,_collocationOptimized=%d,_cacheConnection=%d," \
        "_preferSecure=%d,_endpointSelection=%d,_locatorCacheTimeout=%d.", 
        instance->identityToString(id).c_str(),facet.c_str(),mode,secure,_endpoints.size(),_adapterId.c_str(),
        _locatorInfo.get(),_routerInfo.get(),_collocationOptimized,_cacheConnection,_preferSecure,
        _endpointSelection,_locatorCacheTimeout);*/
...
{
	OUTPUT_LOG_B(0, "getRouterInfo return _routerInfo-P:" << _routerInfo.get())
    return _routerInfo;
}
...
bool IceInternal::RoutableReference::isCTConnectionTest() const
{
	if (this->_flagCTConnectionTest != 0)
	{// inited
		return this->_flagCTConnectionTest > 0;
	}

	// this value is not change, so needn't lock
	//string slctStr = this->getInstance()->initializationData().properties->getPropertyWithDefault("CRS.ICE.EndpointSelection", "CRSConnectionTest");
	string slctStr = this->getInstance()->initializationData().properties->getProperty("CRS.ICE.EndpointSelection");
	if ("CRSConnectionTest" == slctStr)
	{
		this->_flagCTConnectionTest = 1;
	}
	else
	{
		this->_flagCTConnectionTest = -1;
...
#if   1 // jp
	if(newRouterInfo.get() != NULL)
	  r->_overrideTimeout=false;
#endif

	OUTPUT_LOG_B(0, "After changeRouter, r->_routerInfo-P:" << r->_routerInfo.get() << " newRouterInfo-P:" << newRouterInfo.get())
...
#if  1 // jp
	if (_routerInfo.get() != NULL)
		r->_overrideTimeout = true;//如果使用了router，则这个timeout不生效（否则router的connection会不生效）
//	else
...
IceInternal::RoutableReference::getConnection(bool& comp) const
{
    ...
    while(true)
    {
        ...
        if(_locatorInfo)
        {
            //LOG(CSystemLog::LOG_INFO, "[RoutableReference::getConnection] %s:[%s] to get endpoints.",
            //    getInstance()->identityToString(getIdentity()).c_str(), toString().c_str());
        ...
        catch(const LocalException& ex)
        {
            ...
            if(cached)
            {
                ...
                LOG(CSystemLog::LOG_ERROR, "[RoutableReference::getConnection] connection to cached endpoints failed removing endpoints from cache and trying one more time:%s.",ex.what());
                continue;
            }
            throw;
        }
...
IceInternal::RoutableReference::getConnectionNoRouterInfo(const GetConnectionCallbackPtr& callback) const
{
            ...
            setException(const Ice::LocalException& exc)
            {
                ...
                catch(const Ice::LocalException& ex)
                {
                    ...
                    if(_cached)
                    {
                        ...
                        LOG(CSystemLog::LOG_ERROR, "connection to cached endpoints failed removing endpoints from cache and trying one more time");
    ...
    if(_locatorInfo)
    {
        ...
        //LOG(CSystemLog::LOG_INFO, "[RoutableReference::getConnection] %s:[%s] to get endpoints,_locatorCacheTimeout:%d.",
        //    getInstance()->identityToString(getIdentity()).c_str(), toString().c_str(),_locatorCacheTimeout);
...
IceInternal::RoutableReference::createConnection(const vector<EndpointIPtr>& allEndpoints, bool& comp) const
{
    ...
    if(getCacheConnection() || endpoints.size() == 1)
    {
        ...
        //
		connection = factory->create(endpoints, false, getEndpointSelection(), comp, _routerInfo.get()!=NULL?false:_overrideTimeout);
    }
    else
    {
        //
        ...
        for(vector<EndpointIPtr>::const_iterator p = endpoints.begin(); p != endpoints.end(); ++p)
        {
            ...
            {
                endpoint.back() = *p;
				connection = factory->create(endpoint, p + 1 == endpoints.end(), getEndpointSelection(), comp, _routerInfo.get() != NULL ? false : _overrideTimeout);
                break;
            }
            catch(const LocalException& ex)
            {
...
const GetConnectionCallbackPtr& callback) const
{
    ...
    if(getCacheConnection() || endpoints.size() == 1)
    {
        ...
        // existing connection to one of the given endpoints.
        //
		factory->create(endpoints, false, getEndpointSelection(), new CB1(_routerInfo, callback), _routerInfo.get() != NULL ? false : _overrideTimeout);
        return;
    }
    else
    {
        ...
        public:
            ...
            setException(const Ice::LocalException& ex)
            {
				...
				factory->create(endpoint, more, _reference->getEndpointSelection(), this, _overrideTimeout);
            }
            
            CB2(const RoutableReferencePtr& reference, const vector<EndpointIPtr>& endpoints,
				const GetConnectionCallbackPtr& callback, bool overrideTimeout) :
        ...
        RoutableReference* self = const_cast<RoutableReference*>(this);
		factory->create(endpt, true, getEndpointSelection(), new CB2(self, endpoints, callback, _routerInfo.get() != NULL ? false : _overrideTimeout), _routerInfo.get() != NULL ? false : _overrideTimeout);
        return;
    }
}
...
IceInternal::RoutableReference::filterEndpoints(const vector<EndpointIPtr>& allEndpoints) const
{
    ...
    //
	// OUTPUT_LOG_A(1, "getEndpointSelection@filterEndpoints = " << getEndpointSelection());

	//this->getInstance()->initializationData().properties->getPropertyWithDefault()

	if (this->isCTConnectionTest())
	{

#ifdef   OS_WIN
		try
		{
			CRS::ResortEndpointsByNetTest(endpoints);
		}
		catch (...)
		{// resort failed! keep original order
			OUTPUT_LOG_A(1, "ResortEndpointsByNetTest Throw Exception!!");
		}
#endif
