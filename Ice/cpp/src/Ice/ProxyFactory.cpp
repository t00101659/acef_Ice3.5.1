...
IceInternal::ProxyFactory::checkRetryAfterException(const LocalException& ex,
                                                    ...
                                                    int& cnt) const
{
	std::string strLog="checkRetryAfterException";
	WriteLog(strLog);
    ...
    if(one)
    {
        if(ref->getRouterInfo() &&  one->operation.find("ice_add_proxy")==0)
        {
			std::string strLog="checkRetryAfterException ice_add_proxy";
		    WriteLog(strLog);
            //
	...
	else if (dynamic_cast<const Ice::NotRegisteredException*>(&ex))
	{
		ex.ice_throw();
	}
    ...
    _instance(instance)
{
    ...
    else
    {
        for(StringSeq::const_iterator p = retryValues.begin(); p != retryValues.end(); ++p)
        {
            ...
            LOG(CSystemLog::LOG_SYSTEM, "[IceInternal::ProxyFactory::ProxyFactory] _retryIntervals:%d.", v);
