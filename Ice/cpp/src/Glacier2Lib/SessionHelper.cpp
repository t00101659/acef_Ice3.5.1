...
public:
    ...
    SessionHelperI(const Glacier2::SessionThreadCallbackPtr&, const Glacier2::SessionCallbackPtr&,
                   const Ice::InitializationData&, const string&, const Ice::CommunicatorPtr&);
    void destroy();
    ...
    Glacier2::RouterPrx router() const;
...
void SessionHelperI::destroyInternal()
{
    LOG(CSystemLog::LOG_SYSTEM, "SessionHelperI::destroyInternal.");
...
SessionHelperI::destroy()
{
    ...
    _session = 0;
    LOG(CSystemLog::LOG_SYSTEM, "SessionHelperI::destroy.");
    _connected = false;
    _threadCB = 0;
...
SessionHelperI::destroyInternal(const Ice::DispatcherCallPtr& disconnected)
{
    ...
    //first to notify uplevel to downline
    ...
    if(router)
    {
        try
        {
            LOG(CSystemLog::LOG_SYSTEM, "[SessionHelperI::destroyInternal] router:%s will be destroyed.", 
                router->ice_toString().c_str());
            //maybe spend a long time about 42 seconds
...
public:
    ...
    run()
    {
        ...
        LOG(CSystemLog::LOG_DEBUG, "ConnectFailed::run Exception[%s],_callback:0x%p _session:0x%p.", 
            ex->what(), _callback.get(), _session.get());
...
public:
    ...
    run()
    {
        ...
        try
        {
            LOG(CSystemLog::LOG_SYSTEM, "ConnectThread _finder:%s.", _finder.c_str());
            router = Glacier2::RouterPrx::uncheckedCast(_communicator->stringToProxy(_finder));
            LOG(CSystemLog::LOG_DEBUG, "ConnectThread routerPrx:%s.", router->ice_toString().c_str());
            session = _factory->connect(router);
            LOG(CSystemLog::LOG_SYSTEM, "ConnectThread get SessionPrx:%s.", session->ice_toString().c_str());
            _session->connected(router, session);
            LOG(CSystemLog::LOG_SYSTEM, "ConnectThread server connected successfully!");
        }
        catch (const Glacier2::CannotCreateSessionException& ex)
        {
            LOG(CSystemLog::LOG_ERROR, "ConnectThread cannot create session [%s].", ex.reason.c_str());
			if (ex.reason == "session exists")
            {
                try
                {
                    _session->connected(router, session);
                ...
                {
                    //if (dynamic_cast<const Ice::AlreadyRegisteredException*>(&ex))
                    //{
                    //    LOG(CSystemLog::LOG_ERROR, "ConnectThread ex [%s].", ex.what());
                    //    
                    ...
                    //else
                    {
                        try
                        {
                            Ice::DispatcherCallPtr disconnected = new Disconnected(_session, _callback);
                            _session->destroyInternal(disconnected);
        ...
        {
            LOG(CSystemLog::LOG_ERROR, "ConnectThread connect router exception:%s, _callback:0x%p.", 
                ex.what(), _callback.get());
            try
            {
                _session->destroyInternal();
            }
...
public:
    ...
    run()
    {
        try
        {
            LOG(CSystemLog::LOG_SYSTEM, "Connected:_callback:0X%p.", _callback.get());
            _callback->connected(_session);
        }
        catch(const Glacier2::SessionNotExistException&)
        {
            LOG(CSystemLog::LOG_ERROR , "Connected::run exception.");
            _session->destroy();
        }
    }
...
public:
    ...
    {
        LOG(CSystemLog::LOG_ERROR, "connection:%s is closed.", con->toString().c_str());
        _sessionHelper->destroyInternal();
    }
...
SessionHelperI::connected(const Glacier2::RouterPrx& router, const Glacier2::SessionPrx& session)
{
    ...
    _adapter->activate();
    //}
    Ice::EndpointSeq EndSeq = _adapter->getPublishedEndpoints();
    for (size_t i = 0; i < EndSeq.size(); i++)
    {
        LOG(CSystemLog::LOG_SYSTEM, "[SessionHelperI::connected] adapter endpoint:%s.", EndSeq[i]->toString().c_str());
    }
    bool destroy;
    {
        IceUtil::Mutex::Lock sync(_mutex);
        ...
        if(!_destroy)
        {
            ...
            if(acmTimeout > 0)
            {
                ...
                LOG(CSystemLog::LOG_SYSTEM, "[SessionHelperI::connected] setACM connection:%s,acmTimeout:%d.",
                    connection->toString().c_str(), acmTimeout);
            }
        }
    }
...
SessionHelperI::dispatchCallback(const Ice::DispatcherCallPtr& call, const Ice::ConnectionPtr& conn)
{
    ...
    {
        LOG(CSystemLog::LOG_DEBUG, "SessionHelperI::dispatchCallback conn:%s.", conn?conn->toString().c_str():"null");
        call->run();
    }
}
...
Glacier2::SessionFactoryHelper::SessionFactoryHelper(const SessionCallbackPtr& callback, 
                                                    const Ice::CommunicatorPtr& comptr) :
...
{
    LOG(CSystemLog::LOG_SYSTEM, "[SessionFactoryHelper::addThread] session:0x%p %s:%d.",
        session, _routerHost.c_str(), _port);
    //
    ...
    if(previous)
    {
        ...
        //
#ifdef WIN32
        LOG(CSystemLog::LOG_ERROR, "Join previous thread session:0x%p,oldthread:0x%p,threadid:%6x.", 
            session, previous.get(), previous->getThreadControl().id());
#else
        LOG(CSystemLog::LOG_ERROR, "Join previous thread session:0x%p,oldthread:0x%p,threadid:%6lx.", 
            session, previous.get(), previous->getThreadControl().id());
#endif
...
*/
void
Glacier2::SessionFactoryHelper::reconnect(const Glacier2::SessionHelperPtr& session, const string& user,  
const string& password)
{
    SessionHelperIPtr newsession = SessionHelperIPtr::dynamicCast(session);
    map<string, string> context;
    {
        IceUtil::Mutex::Lock sync(_mutex);
        context = _context;
    }
    newsession->connect(user, password, context);
    return;
}
...
{
    LOG(CSystemLog::LOG_SYSTEM, "[SessionFactoryHelper::connect] connect %s:%s.",
...
Glacier2::SessionFactoryHelper::createInitData()
{
    ...
    if(initData.properties->getProperty("Ice.Default.Router").size() == 0 && !_identity.name.empty())
    {
        ...
        LOG(CSystemLog::LOG_INFO, "[SessionFactoryHelper::createInitData] Ice.Default.Router:%s.",
            createProxyStr(_identity).c_str());
    } 
    else
        LOG(CSystemLog::LOG_INFO, "[SessionFactoryHelper::createInitData] default Ice.Default.Router:%s.",
            createProxyStr(_identity).c_str());
#ifndef ICE_OS_WINRT
    //
